...
3. Đồng nhất tín hiệu (so sánh kết quả LONG/SHORT/NEUTRAL)

Usage:
    python benchmark_atc_comparison.py
    python benchmark_atc_comparison.py --simd  # Include SIMD benchmark
...
def run_benchmark(include_simd: bool = False):
    ...
    if include_simd:
        print("              + SIMD Optimization Comparison")
    print("=" * 80)
    print()
    ...
    for symbol in SYMBOLS:
        for tf in TIMEFRAMES:
            ...
            if simd_available:
                ...
                simd_result = benchmark_rust_module(symbol, tf, data)
                simd_result["module"] = "Rust (SIMD)"
    ...
    # Print summary
    if simd_available:
        print_summary_with_simd(python_results, rust_results, rust_simd_results)
    else:
...
def print_summary_with_simd(python_results: list, rust_results: list, rust_simd_results: list):
    """Print benchmark summary including SIMD comparison."""
    print()
    print("=" * 80)
    print("BENCHMARK SUMMARY (WITH SIMD)")
    print("=" * 80)
    print()
    ...
    print("1. SPEED COMPARISON")
    print("-" * 100)
    print(f"{'Symbol':<12} {'TF':<6} {'Python (ms)':<18} {'Rust (ms)':<18} {'Rust SIMD (ms)':<18} {'Speedup':<12}")
    print("-" * 100)
    ...
    for py, rust, simd in zip(python_results, rust_results, rust_simd_results):
        ...
        print(
            ...
            f"{speedup_rust:>5.1f}x / {speedup_simd:>5.1f}x"
        )
    ...
    print(
        ...
        f"{avg_speedup_rust:>5.1f}x / {avg_speedup_simd:>5.1f}x"
    )
    print()
    print(f"Average Speedup: Rust={avg_speedup_rust:.2f}x, SIMD={avg_speedup_simd:.2f}x")
    print(f"SIMD vs Scalar Rust: {simd_vs_scalar:.2f}x")
    print()

    # SIMD Speedup Analysis
    print("2. SIMD OPTIMIZATION IMPACT")
    print("-" * 80)
    print(f"{'Symbol':<12} {'TF':<6} {'Scalar (ms)':<15} {'SIMD (ms)':<15} {'SIMD Speedup':<15}")
    print("-" * 80)

    simd_speedups = []
    for rust, simd in zip(rust_results, rust_simd_results):
    ...
    print(f"SIMD Speedup Statistics:")
    ...
    print("-" * 80)
    print(f"{'Symbol':<12} {'TF':<6} {'Python':<10} {'Rust':<10} {'SIMD':<10} {'Match':<10}")
    print("-" * 80)
    ...
    for py, rust, simd in zip(python_results, rust_results, rust_simd_results):
        match = "YES" if py["signal_type"] == rust["signal_type"] == simd["signal_type"] else "NO"
        if py["signal_type"] == rust["signal_type"] == simd["signal_type"]:
            matches += 1
        total += 1

        print(
            f"{py['symbol']:<12} {py['timeframe']:<6} {py['signal_type']:<10} "
            f"{rust['signal_type']:<10} {simd['signal_type']:<10} {match:<10}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--simd", action="store_true", help="Include SIMD benchmark")
    args = parser.parse_args()

    run_benchmark(include_simd=args.simd)
