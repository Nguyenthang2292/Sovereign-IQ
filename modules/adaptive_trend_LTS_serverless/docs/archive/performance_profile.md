...
This project includes configurable build profiles for different use cases:

### Profile: `release` (Default for Production)
...
**Use case**: Production Lambda deployment
**Binary size**: Minimal (no debug symbols)

### Profile: `release-debug` (For Troubleshooting)

```toml
[profile.release-debug]
inherits = "release"  # Inherit all release settings
debug = true           # Add debug symbols
```

**Use case**: Debugging production issues, troubleshooting crashes
**Binary size**: Slightly larger (includes debug symbols)

### Binary Size Comparison

| Profile | Debug Symbols | Expected Size | Notes |
|---------|--------------|---------------|-------|
| `release` | No | ~12-15 MB | Optimized for production |
| `release-debug` | Yes | ~14-18 MB | +2-3 MB for debugging |

**Note**: The actual binary size depends on dependencies. Rust binaries with many dependencies can include significant debug info.

### Building

```bash
# Production build (no debug symbols) - DEFAULT
cargo build --release
...
# Lambda-specific
cd lambda
cargo lambda build --release                    # Production
cargo lambda build --profile release-debug     # With debug symbols
```

### When to Use Each Profile

**Use `release` (default) when:**
- Deploying to production
- Binary size matters (Lambda 250MB limit)
- Maximum performance is needed

**Use `release-debug` when:**
- Investigating crashes or panics
- Need stack traces with line numbers
- Debugging production issues with core dumps
