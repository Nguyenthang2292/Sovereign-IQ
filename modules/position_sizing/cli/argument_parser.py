...
def _configure_symbol_source(config):
    ...
    while True:
        ...
        source_choice, action = _prompt_with_back_ps(
            f"Select source (a/b/c/d/e, {default_source_text}): ", default=current_source
        )
        ...
        if source_choice == "a":
            ...
            while True:
                symbols_str, action = _prompt_with_back_ps(
                    "Enter symbols (comma-separated, e.g., btc,eth or BTC/USDT,ETH/USDT): "
                )
                if action == "main":
                    return "main"
...
def _configure_account_balance(config):
    ...
    while True:
        ...
        balance_choice, action = _prompt_with_back_ps(
            f"Select option (a/b/c, {default_balance_text}): ", default=current_choice
        )
        ...
        if balance_choice == "a":
            ...
            try:
                ...
                if config["account_balance"] <= 0:
                    print(color_text("Invalid balance. Using default: 10000 USDT", Fore.YELLOW))
...
def _configure_backtest_settings(config):
    ...
    while True:
        ...
        auto_tf_choice, action = _prompt_with_back_ps(
            "Select option (a/b/c, default=b): ", default="a" if current_auto_tf else "b"
        )
        ...
        if not config["auto_timeframe"]:
            ...
            timeframe, action = _prompt_with_back_ps(
                f"Timeframe ({color_text(f'default: {DEFAULT_TIMEFRAME}', Fore.MAGENTA)}): ", default=current_tf
            )
            if action == "main":
                return "main"
        ...
        lookback_str, action = _prompt_with_back_ps(
            f"Lookback days ({color_text(f'default: {DEFAULT_LOOKBACK_DAYS}', Fore.MAGENTA)}): ",
        ...
        try:
            ...
            if config["lookback_days"] <= 0:
                print(
                    color_text(
                        f"Invalid lookback days (must be >= 1). Using default: {DEFAULT_LOOKBACK_DAYS}", Fore.YELLOW
                    )
                )
...
def _configure_position_size(config):
    ...
    while True:
        ...
        print("   Note: Max position size is a fraction of account balance")
        print("   Example: 0.1 = 10% of account, 0.2 = 20% of account")
        ...
        default_max_size_text = color_text(
            f"default: {DEFAULT_MAX_POSITION_SIZE} = {DEFAULT_MAX_POSITION_SIZE * 100:.0f}%", Fore.MAGENTA
        )
        max_size_str, action = _prompt_with_back_ps(
            f"Max position size (fraction, {default_max_size_text}): ", default=current_max_size
        )
