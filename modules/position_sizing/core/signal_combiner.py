...
class SignalCombinerMixin:
    ...
    def combine_signals_majority_vote(
        ...
        if not isinstance(self.min_indicators_agreement, int):
            ...
            raise AttributeError(
                f"min_indicators_agreement must be >= 1, got {self.min_indicators_agreement}"
            )
        ...
        if expected_signal_type_upper == "LONG":
            ...
            raise ValueError(
                f"Invalid expected_signal_type: '{expected_signal_type}'. "
                f"Must be 'LONG' or 'SHORT' (case-insensitive)."
            )
