    ...
    def __init__(
        ...
        if default_confidence is not None:
            if not isinstance(default_confidence, (int, float)) or not (0.0 <= default_confidence <= 1.0):
                raise ValueError(
                    f"default_confidence must be a float between 0.0 and 1.0, got {default_confidence}"
                )
        ...
        if not isinstance(min_indicators_agreement, int):
            raise ValueError(
                f"min_indicators_agreement must be an int, got {type(min_indicators_agreement).__name__}"
            )
    ...
    def _calculate_indicators_sequential(
        ...
        if 'range_oscillator' in self.enabled_indicators:
            try:
                ...
                log_warn(f"range_oscillator signal calculation failed: symbol={symbol}, timeframe={timeframe}, period_index={period_index}, error={e}")
        ...
        if 'spc' in self.enabled_indicators:
            ...
            for strategy in spc_strategies:
                try:
                    ...
                    log_warn(f"{indicator_name} signal calculation failed: symbol={symbol}, timeframe={timeframe}, period_index={period_index}, error={e}")
        ...
        if 'xgboost' in self.enabled_indicators and historical_df_len >= 50:
            try:
                ...
                log_warn(f"xgboost signal calculation failed: symbol={symbol}, timeframe={timeframe}, period_index={period_index}, error={e}")
        ...
        if 'hmm' in self.enabled_indicators:
            try:
                ...
                log_warn(f"hmm signal calculation failed: symbol={symbol}, timeframe={timeframe}, period_index={period_index}, error={e}")
        ...
        if 'random_forest' in self.enabled_indicators:
            try:
                ...
                log_warn(f"random_forest signal calculation failed: symbol={symbol}, timeframe={timeframe}, period_index={period_index}, error={e}")
        ...
        if 'spc' in self.enabled_indicators:
            ...
            for strategy in spc_strategies:
                try:
                    ...
                    indicators[indicator_name] = pd.DataFrame({
                        'signal': np.nan,
                        'confidence': np.nan
                    }, index=df.index)
