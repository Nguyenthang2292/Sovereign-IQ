    ...
    def calculate_position_size(
        ...
        **kwargs,
        ...
        try:
            ...
            log_progress(
                f"    Metrics: win_rate={metrics.get('win_rate', 0.0):.2%}, "
            ...
            if equity_curve is not None and len(equity_curve) > 0:
                try:
                    ...
                    if initial_capital > 0:
                        ...
                        cumulative_performance_multiplier = (
                            confidence * raw_multiplier + (1.0 - confidence) * capped_multiplier
                        )

                        log_progress(
                            f"  Cumulative performance: {cumulative_performance * 100:.2f}%, "
            ...
            if kelly_fraction == 0.0:
                ...
                adjusted_kelly_fraction = max(
                    self.min_position_size, min(self.max_position_size, adjusted_kelly_fraction)
                )
            ...
            log_progress(
                f"  Completed: {symbol} - Position size: {position_size_usdt:.2f} USDT ({position_size_pct:.2f}%)"
            )
