...
class BayesianKellyCalculator:
        ...
        try:
            ...
            try:
                with open(log_path, 'a', encoding='utf-8') as f:
                    f.write(json.dumps({"id": f"log_kelly_calc_start_{os.getpid()}", "timestamp": pd.Timestamp.now().timestamp() * 1000, "location": "kelly_calculator.py:104", "message": "calculate_kelly_fraction start", "data": {"win_rate": win_rate, "avg_win": avg_win, "avg_loss": avg_loss, "num_trades": num_trades, "min_trades": self.min_trades, "min_win_rate": self.min_win_rate}, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "A"}) + "\n")
            except: pass
            # #endregion
            ...
            try:
                with open(log_path, 'a', encoding='utf-8') as f:
                    f.write(json.dumps({"id": f"log_kelly_wins_losses_{os.getpid()}", "timestamp": pd.Timestamp.now().timestamp() * 1000, "location": "kelly_calculator.py:110", "message": "calculated wins and losses", "data": {"num_wins": num_wins, "num_losses": num_losses}, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "A"}) + "\n")
            except: pass
            # #endregion
            ...
            if kelly_fraction < 0:
                log_warn(f"Negative Kelly fraction calculated ({kelly_fraction:.4f}). Returning 0.0")
                return 0.0
    ...
    def calculate_kelly_from_metrics(
        ...
        try:
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps({"id": f"log_kelly_input_{os.getpid()}", "timestamp": pd.Timestamp.now().timestamp() * 1000, "location": "kelly_calculator.py:205", "message": "calculate_kelly_from_metrics input", "data": {"metrics_keys": list(metrics.keys()) if metrics else [], "metrics_type": str(type(metrics)), "metrics_is_none": metrics is None}, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "A"}) + "\n")
        except: pass
        # #endregion
        ...
        try:
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps({"id": f"log_kelly_values_{os.getpid()}", "timestamp": pd.Timestamp.now().timestamp() * 1000, "location": "kelly_calculator.py:212", "message": "extracted metrics values", "data": {"win_rate": win_rate, "avg_win": avg_win, "avg_loss": avg_loss, "num_trades": num_trades}, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "A"}) + "\n")
        except: pass
        # #endregion
        ...
        try:
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps({"id": f"log_kelly_result_{os.getpid()}", "timestamp": pd.Timestamp.now().timestamp() * 1000, "location": "kelly_calculator.py:222", "message": "kelly fraction result", "data": {"kelly_fraction": result, "result_type": str(type(result)), "result_is_nan": pd.isna(result) if hasattr(pd, 'isna') else (result != result)}, "sessionId": "debug-session", "runId": "run1", "hypothesisId": "A"}) + "\n")
        except: pass
        # #endregion
