...
def ma_calculation_rust(
    ...
    try:
        ...
        def calculate_rust_ma(price_data=None, p_length=None):
            ...
            if ma == "EMA":
                result = calculate_ema(price_data, p_length, use_rust=use_rust)
            elif ma == "WMA":
                result = calculate_wma(price_data, p_length, use_rust=use_rust)
            elif ma == "DEMA":
                result = calculate_dema(price_data, p_length, use_rust=use_rust)
            elif ma == "LSMA":
                result = calculate_lsma(price_data, p_length, use_rust=use_rust)
            elif ma == "HMA":
                result = calculate_hma(price_data, p_length, use_rust=use_rust)
            elif ma == "KAMA":
                result = calculate_kama(price_data, p_length, use_rust=use_rust)
            else:
                return None
