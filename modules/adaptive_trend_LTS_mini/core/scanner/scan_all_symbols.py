...
def scan_all_symbols(
    ...
    Args:
        ...
        max_symbols: Maximum number of symbols to scan (None = all symbols). Ignored if symbols is provided.
        ...
        symbols: Optional list of symbols to scan. If provided, only these symbols are scanned
                (e.g. pre-filter Stage 0 output); exchange discovery is skipped.
    ...
    try:
        ...
        if symbols is not None:
            # Pre-filter mode: use provided symbol list (e.g. Stage 0 output)
            if not symbols:
