...
def validate_atc_inputs(
    ...
    if cutout >= len(prices):
        log_warn(f"cutout ({cutout}) >= prices length ({len(prices)}), capping at length-1")
        cutout = len(prices) - 1

    # Check index alignment between prices and src
    if not prices.index.equals(src.index):
        intersection = prices.index.intersection(src.index)
        overlap_pct = (len(intersection) / len(prices)) * 100
        if overlap_pct < 90:
            log_warn(
                f"Index mismatch: prices and src overlap only {overlap_pct:.1f}%. "
                f"This may lead to significant data loss or incorrect signals."
            )
        else:
            log_debug(f"Minor index mismatch between prices and src ({overlap_pct:.1f}% overlap). Aligning...")
