...
def _process_partition_with_rust_cuda(
    ...
    try:
        ...
        batch_results = atc_rust.compute_atc_signals_batch(
            symbols_numpy,
            ema_len=params.get("ema_len", 28),
            hull_len=params.get("hma_len", 28),  # Rust uses 'hull_len' parameter
            wma_len=params.get("wma_len", 28),
            dema_len=params.get("dema_len", 28),
            lsma_len=params.get("lsma_len", 28),
            kama_len=params.get("kama_len", 28),
...
def process_symbols_rust_dask(
    ...
    def _prepare_partition(items: list) -> Dict[str, pd.Series]:
        """Convert partition items to dict while preserving pandas Series with indices."""
        partition_dict = {}
        for symbol, prices in items:
            ...
            if isinstance(prices, pd.Series):
                if prices.empty:
                    continue
                # CRITICAL FIX: Keep as Series to preserve index for comparison
                partition_dict[symbol] = prices
            elif isinstance(prices, np.ndarray):
                if prices.size == 0:
                    continue
                # Convert to Series with default index
                partition_dict[symbol] = pd.Series(prices)
            else:
                partition_dict[symbol] = pd.Series(prices)
        return partition_dict

    def _process_partition(partition_dict: Dict[str, pd.Series]) -> Dict[str, Dict[str, pd.Series]]:
    ...
    # CRITICAL FIX: Convert items list to dict properly, then process
    # map_partitions receives iterator of (symbol, prices) tuples per partition
    results_bag = symbols_bag.map_partitions(lambda items: [_process_partition(_prepare_partition(list(items)))])
