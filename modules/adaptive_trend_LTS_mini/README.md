...
# Compute ATC signals (CPU-only)
result = compute_atc_signals(
    prices,
    use_rust_backend=True,  # Recommended
    precision="float64"
)
...
### Differences from Full GPU Version

| Aspect | GPU Version | CPU-Only Version |
|--------|-------------|------------------|
| Speed | 10-50ms per symbol | 100-500ms per symbol |
| Batch 100 symbols | 2-10s | 10-50s |
| Memory | Uses GPU VRAM | Uses system RAM |
| Dependencies | CUDA, cuDNN | None extra |
| Hardware | NVIDIA GPU required | Any CPU |
...
Remove `use_cuda` from your config:
...
Python API Layer
    ↓
Rust/Rayon Backend (CPU Parallelism)
    ↓
...
- ❌ **cudarc** Rust dependency (v0.19 with CUDA 12.8 features)
- ❌ **CuPy** Python package dependency
- ❌ **PyCUDA** Python package dependency
...
##### Rust CUDA Implementation Files
...
- ❌ `rust_extensions/build.rs` (CUDA-specific)

##### Python GPU Backend Files
...
- ✅ Removed `--use-cuda` CLI flag from argument parser
- ✅ Removed `use_cuda` field from `ATCConfig` dataclass

##### Rust Extension Changes
- ✅ `Cargo.toml`: Removed cudarc dependency
- ✅ `src/lib.rs`: Removed CUDA module declarations and function exports
- ✅ Build size reduced: ~1.5MB → ~637KB (57% smaller)

##### Routing Logic Simplification
- ✅ `batch_processor.py`: Removed CUDA routing, now CPU-only
- ✅ `dask_batch_processor.py`: Removed CUDA backend option
- ✅ `rust_backend.py`: Removed all CUDA wrapper functions
...
#### Validation Results
- ✅ Rust extension builds successfully without CUDA
