...
def _get_trend_direction(trend_value: float) -> tuple[str, str]:
    """Get trend direction label and color based on trend value.

    Args:
        trend_value: Numeric trend value

    Returns:
        Tuple of (direction_label, color_code)
    """
    if trend_value > 0:
        return "BULLISH", Fore.GREEN
    elif trend_value < 0:
        return "BEARISH", Fore.RED
    else:
        return "NEUTRAL", Fore.YELLOW


def _display_ma_signals(ma_signals: list[tuple[str, pd.Series]]) -> None:
    """Display individual MA signals with trend indicators.

    Args:
        ma_signals: List of (MA_name, signal_series) tuples
    """
    print(color_text("Individual MA Signals:", Fore.MAGENTA, Style.BRIGHT))

    for ma_name, ma_sig in ma_signals:
        # Cache empty check
        is_empty = ma_sig.empty
        if not is_empty:
...
def _display_equity_weights(ma_weights: list[tuple[str, pd.Series]]) -> None:
    """Display equity weights for each MA.

    Args:
        ma_weights: List of (MA_name, weight_series) tuples
    """
    print(color_text("Equity Weights (Layer 2):", Fore.MAGENTA, Style.BRIGHT))

    for ma_name, ma_weight in ma_weights:
        # Cache empty check
        is_empty = ma_weight.empty
        if not is_empty:
...
def display_atc_signals(
    ...
    # Validate non-empty series before accessing
    if len(average_signal) == 0:
        log_error("Empty ATC signal series")
        return
    ...
    trend_direction, trend_color = _get_trend_direction(latest_trend_value)

    # Display header
    print("\n" + color_text("=" * DISPLAY_WIDTH, Fore.CYAN, Style.BRIGHT))
    print(
        color_text(
    ...
    print(color_text("=" * DISPLAY_WIDTH, Fore.CYAN, Style.BRIGHT))
...
def display_scan_results(long_signals: pd.DataFrame, short_signals: pd.DataFrame, min_signal: float) -> None:
    """
    ...
    if long_signals.empty:
        ...
        for _, row in long_signals.iterrows():
            ...
            print(color_text(row_text, Fore.GREEN))
    ...
    if short_signals.empty:
        ...
        for _, row in short_signals.iterrows():
            ...
            print(color_text(row_text, Fore.RED))
...
def list_futures_symbols(data_fetcher: DataFetcher, max_symbols: Optional[int] = None) -> None:
    """
