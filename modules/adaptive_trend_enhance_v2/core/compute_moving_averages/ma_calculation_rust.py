...
def ma_calculation_rust(
    ...
    try:
        ...
        if ma == "EMA":
            result = calculate_ema(prices_np, length, use_rust=use_rust)
        elif ma == "WMA":
            result = calculate_wma(prices_np, length, use_rust=use_rust)
        elif ma == "DEMA":
            result = calculate_dema(prices_np, length, use_rust=use_rust)
        elif ma == "LSMA":
            result = calculate_lsma(prices_np, length, use_rust=use_rust)
        elif ma == "HMA":
            result = calculate_hma(prices_np, length, use_rust=use_rust)
        elif ma == "KAMA":
            result = calculate_kama(prices_np, length, use_rust=use_rust)
        else:
            return None
