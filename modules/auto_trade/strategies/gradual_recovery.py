...
class GradualRecoveryStrategy:
    def __init__(self, initial_loss: float, config: RecoveryConfig, database: Optional[Any] = None) -> None:
        self.initial_loss: float = initial_loss
        self.config: Dict[str, Any] = self._validate_config(config)
        self.database: Optional[Any] = database
    ...
    def calculate_next_position_size(self) -> float:
        mode: str = str(self.config.get("margin_scaling_mode", "fixed"))
        remaining: float = float(self._state["remaining_loss"])
        ...
        target_pct: float = float(self.config.get("target_profit_per_trade", 5.0)) / 100
        return remaining / target_pct if target_pct > 0 else remaining

    def calculate_next_leverage(self) -> int:
        mode: str = str(self.config.get("leverage_scaling_mode", "fixed"))
        ...
        return int(self.config.get("min_leverage", 2))

    def estimate_remaining_trades(self) -> int:
        target_pct: float = float(self.config.get("target_profit_per_trade", 5.0)) / 100
        trades: int = int(self._state["trades_count"])

        estimated_profit: float
        if trades > 0 and self._state["total_profit_accumulated"] > 0:
            avg_profit: float = float(self._state["total_profit_accumulated"]) / trades
            estimated_profit = avg_profit
        else:
            estimated_profit = float(self._state["remaining_loss"]) * target_pct

        return int(self._state["remaining_loss"] / estimated_profit) if estimated_profit > 0 else 0

    def should_stop(self) -> bool:
        max_trades: int = int(self.config.get("max_recovery_trades", 20))
        max_loss: float = float(self.config.get("max_total_loss", 2.0 * self.initial_loss))
    ...
    def get_state(self) -> RecoveryState:
        return RecoveryState(
            ...
            remaining_loss=float(self._state["remaining_loss"]),
            total_profit_accumulated=float(self._state["total_profit_accumulated"]),
            recovery_percentage=self.recovery_percentage,
            trades_count=int(self._state["trades_count"]),
            win_streak=int(self._state["win_streak"]),
            is_complete=bool(self._state["is_complete"]),
    ...
    def _calculate_progressive_margin(self) -> float:
        base_margin: float = self.initial_loss / 10
        recovery_pct: float = self.recovery_percentage / 100
        scaling_factor: float = 0.5
        return base_margin * (1 + recovery_pct * scaling_factor)

    def _calculate_adaptive_margin(self) -> float:
        base_margin: float = self.initial_loss / 10
        win_streak: int = int(self._state["win_streak"])
        streak_bonus: float = 0.1 * min(win_streak, 5)

        recovery_pct: float = self.recovery_percentage / 100
        progress_boost: float = 0.3 * recovery_pct
    ...
    def _calculate_progressive_leverage(self) -> int:
        min_lev: int = int(self.config.get("min_leverage", 2))
        max_lev: int = int(self.config.get("max_leverage", 10))
        recovery_pct: float = self.recovery_percentage / 100

        leverage: int = min_lev + int((max_lev - min_lev) * recovery_pct)
    ...
    def _calculate_adaptive_leverage(self) -> int:
        ...
        base_lev: int = min_lev + int((max_lev - min_lev) * (self.recovery_percentage / 100))
        streak_bonus: int = min(win_streak, 3)

        leverage: int = base_lev + streak_bonus
        return max(min_lev, min(max_lev, leverage))
...
def create_recovery_plan(initial_loss: float, config: RecoveryConfig) -> Dict[str, Any]:
    target_pct: float = float(config.get("target_profit_per_trade", 5.0)) / 100
    profit_per_trade: float = initial_loss * target_pct
    estimated_trades: int = int(initial_loss / profit_per_trade) if profit_per_trade > 0 else 0
    ...
    return {
        ...
        "suggested_leverage_start": int(config.get("min_leverage", 2)),
        "suggested_leverage_end": int(config.get("max_leverage", 10)),
        "max_total_loss": float(config.get("max_total_loss", 2.0 * initial_loss)),
