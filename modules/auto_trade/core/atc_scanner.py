...
class ATCScanner:
    ...
    def _set_cache(self, cache_key: str, data: Dict[str, Any]) -> None:
        ...
        Args:
            cache_key: Cache key generated by _get_cache_key
            data: Scan result data to cache (longs/shorts sets, strengths dict)
        """
        if not self.enable_cache:
            return
        ...
        if self._use_rust_cache and self._rust_cache:
            try:
                ...
                longs: Set[str] = set(data.get("longs", set()))
                shorts: Set[str] = set(data.get("shorts", set()))
        ...
        # Fallback to Python cache with lock (store data + timestamp)
        with self._cache_lock:
