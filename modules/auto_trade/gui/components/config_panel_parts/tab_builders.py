...
def create_ui_preferences_tab(panel):
    """Create UI Preferences tab."""
    tab = panel.tabview.add("UI Preferences")
    ui_frame = ctk.CTkFrame(tab, fg_color="transparent")
    ui_frame.pack(fill="both", expand=True, padx=10, pady=10)

    ctk.CTkLabel(ui_frame, text="Theme:", font=("Arial", 12)).pack(anchor="w", pady=(5, 2))
    panel.theme_var = ctk.StringVar(value="Dark")
    theme_switch = ctk.CTkSegmentedButton(
        ui_frame,
        values=["Dark", "Light"],
        variable=panel.theme_var,
        width=200,
        command=panel._on_theme_change,
    )
    theme_switch.pack(anchor="w", pady=2)

    ctk.CTkLabel(ui_frame, text="Font Size:", font=("Arial", 12)).pack(anchor="w", pady=(10, 2))
    panel.font_size_var = ctk.IntVar(value=12)
    font_size_slider = ctk.CTkSlider(
        ui_frame,
        from_=10,
        to=16,
        number_of_steps=7,
        variable=panel.font_size_var,
        width=300,
        command=panel._on_font_size_change,
    )
    font_size_slider.pack(anchor="w", pady=2)
    panel.font_size_label = ctk.CTkLabel(ui_frame, text=f"Current: {panel.font_size_var.get()}pt", font=("Arial", 10))
    panel.font_size_label.pack(anchor="w", pady=2)

    ctk.CTkLabel(ui_frame, text="Auto-Refresh Intervals (seconds):", font=("Arial", 12)).pack(anchor="w", pady=(15, 2))
    refresh_frame = ctk.CTkFrame(ui_frame, fg_color="transparent")
    refresh_frame.pack(anchor="w", pady=2)

    ctk.CTkLabel(refresh_frame, text="Signals:", font=("Arial", 10), text_color="gray").pack(side="left", padx=(0, 5))
    panel.signal_refresh_entry = ctk.CTkEntry(refresh_frame, placeholder_text="30", width=80)
    panel.signal_refresh_entry.pack(side="left", padx=5)
    panel.signal_refresh_entry.insert(0, "30")

    ctk.CTkLabel(refresh_frame, text="Positions:", font=("Arial", 10), text_color="gray").pack(side="left", padx=(10, 5))
    panel.position_refresh_entry = ctk.CTkEntry(refresh_frame, placeholder_text="10", width=80)
    panel.position_refresh_entry.pack(side="left", padx=5)
    panel.position_refresh_entry.insert(0, "10")
    ...
    export_btn = ctk.CTkButton(
        ...
        fg_color="#4488ff",
        hover_color="#0066ff",
        command=panel._export_settings,
    )
    ...
    import_btn = ctk.CTkButton(
        ...
        fg_color="#44aaff",
        hover_color="#0088ff",
        command=panel._import_settings,
    )
