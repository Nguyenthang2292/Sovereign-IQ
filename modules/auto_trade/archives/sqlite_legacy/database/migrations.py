...
class MigrationManager:
    ...
    def set_version(self, version: str) -> None:
        ...
        log_info(f"Database schema version updated to {version}")
    ...
    def initialize_database(self) -> bool:
        ...
        if not os.path.exists(self.db_path):
            log_info(f"Database does not exist. Creating: {self.db_path}")
            ...
            log_info(f"Database initialized successfully from {self.schema_path}")
    ...
    def enable_wal_mode(self) -> bool:
        ...
        try:
            ...
            log_error(f"Error enabling WAL mode: {e}")
            return False
    ...
    def check_integrity(self) -> bool:
        ...
        try:
            ...
            log_error(f"Error checking database integrity: {e}")
            return False
    ...
    def apply_migration(self, migration_sql: str, migration_name: str) -> bool:
        ...
        """
        try:
            log_info(f"Applying migration: {migration_name}")
            ...
            if migration_name == "003_add_trailing_step_index.sql" and self._column_exists("orders", "trailing_step_index"):
                log_info(f"Migration {migration_name}: column already exists, marking as applied")
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
            ...
            log_info(f"Migration {migration_name} applied successfully")
            return True

        except Exception as e:
            log_error(f"Error applying migration {migration_name}: {e}")
            return False
...
"""
        ...
        log_info(f"Created migration template: {filepath}")
        return str(filepath)
    ...
    def auto_migrate(self) -> bool:
        ...
        for migration_file in pending:
            ...
            if not self.apply_migration(migration_sql, migration_file):
                log_error(f"Failed to apply migration: {migration_file}")
                return False
    ...
    def vacuum_database(self) -> bool:
        ...
        try:
            ...
            log_error(f"Error during VACUUM: {e}")
            return False
    ...
    def analyze_database(self) -> bool:
        ...
        try:
            ...
            log_error(f"Error during ANALYZE: {e}")
            return False
    ...
    def get_table_row_counts(self) -> Dict[str, int]:
        ...
        for table in tables:
            ...
            if table not in ALLOWED_TABLES:
                log_warn(f"Skipping unknown table: {table}")
                continue
...
def initialize_database_if_needed(db_path: str, schema_path: str, auto_migrate: bool = True) -> bool:
    ...
    if manager.initialize_database():
        log_info("Database initialized from schema")
    else:
