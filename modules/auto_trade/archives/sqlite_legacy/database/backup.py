...
class BackupManager:
    ...
    def create_backup(self, backup_name: Optional[str] = None, metadata: Optional[dict] = None) -> Optional[str]:
        ...
        if not os.path.exists(self.db_path):
            log_error(f"Database file not found: {self.db_path}")
            return None
        ...
        try:
            ...
            if self.verify_backup(str(backup_file)):
                ...
                log_error(f"Backup verification failed: {backup_file}")
        ...
        try:
            ...
            if path.suffix == ".gz":
                try:
                    ...
                    log_error(f"Backup compression verification failed: {e}")
                    return False
            else:
                ...
                try:
                    ...
                    log_error(f"Backup SQLite verification failed: {e}")
                    return False

        except Exception as e:
            log_error(f"Error verifying backup: {e}")
            return False
    ...
    def restore_backup(self, backup_path: str, target_path: Optional[str] = None) -> bool:
        ...
        if not path.exists():
            log_error(f"Backup file not found: {path}")
            return False
        ...
        try:
            ...
            if os.path.exists(target_path):
                ...
                log_info(f"Created pre-restore backup: {pre_restore_backup}")

            log_info(f"Restoring from backup: {path}")
            ...
            if result and result[0] == "ok":
                log_info(f"Database restored successfully to {target_path}")
                return True
            else:
                log_error(f"Restored database failed integrity check: {result}")
                return False
    ...
    def cleanup_old_backups(self) -> int:
        ...
        for backup in to_delete:
            try:
                ...
                log_info(f"Deleted old backup: {backup['filename']}")
                deleted_count += 1

            except Exception as e:
                log_error(f"Error deleting backup {backup['filename']}: {e}")
    ...
    def schedule_backup(self) -> bool:
        ...
        try:
            ...
            log_error(f"Error in scheduled backup: {e}")
            return False
    ...
    def export_to_sql(self, output_path: str) -> bool:
        ...
        try:
            ...
            log_info(f"Database exported to SQL: {output_path}")
            return True

        except Exception as e:
            log_error(f"Error exporting to SQL: {e}")
            return False
