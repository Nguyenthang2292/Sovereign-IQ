...
class DatabaseManager:
    ...
    def __init__(self, db_path: str, echo: bool = False, pool_size: int = 5, max_overflow: int = 10) -> None:
        ...
        log_info(f"Database manager initialized: {db_path}")

    def create_all_tables(self) -> None:
        """Create all tables from models."""
        Base.metadata.create_all(bind=self.engine)
        log_info("All database tables created")

    def drop_all_tables(self) -> None:
        """Drop all tables (use with caution!)."""
        Base.metadata.drop_all(bind=self.engine)
        log_warn("All database tables dropped")
    ...
    def optimize_database(self) -> None:
        ...
        with self.engine.connect() as conn:
            ...
            try:
                ...
                log_warn(f"VACUUM failed (may be in WAL mode): {e}")
...
class DataExporter:
    ...
    def export_to_json(
        ...
        try:
            ...
            log_error(f"JSON export failed: {e}")
            return False
...
class DatabaseCleaner:
    ...
    def cleanup_old_records(
        ...
        try:
            ...
            log_info(f"Deleted {deleted} old records from {model_class.__tablename__}")
            return deleted
    ...
    def archive_old_orders(session: Session, days_to_keep: int = 90, archive_path: str = "data/archive") -> int:
        ...
        try:
            ...
            log_info(f"Archived {len(old_orders)} orders to {archive_file}")
...
def reset_database_for_testing(db_manager: DatabaseManager) -> None:
    ...
    """
    log_warn("Resetting database - all data will be deleted!")
...
def export_all_data(session: Session, output_dir: str = "data/exports") -> bool:
    ...
    log_info(f"All data exported to {output_dir}")
    return True
