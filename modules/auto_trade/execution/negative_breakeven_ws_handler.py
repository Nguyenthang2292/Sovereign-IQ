                        ...
                        if self.binance_client:
                            try:
                                ...
                                if success:
                                    ...
                                    logger.info(
                                        f"[WS] Negative breakeven triggered for {order.symbol} {order.side}: "
                                        f"TP {old_tp} → {new_tp} (entry price)"
                                    )
                                else:
                                    error_msg = (modify_result or {}).get("error", "Unknown error")
                                    logger.error(f"[WS] Failed to modify TP for {order.order_id}: {error_msg}")

                            except Exception as e:
                                logger.error(f"[WS] Error modifying TP for {order.order_id}: {e}")
                        else:
                            # Dry run - just log
                            logger.info(
                                f"[WS] Negative breakeven would trigger for {order.symbol}: "
                                f"TP {order.take_profit} → {new_tp}"
                            )
