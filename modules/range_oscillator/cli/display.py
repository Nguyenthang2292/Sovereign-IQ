...
def display_final_results(
    ...
    if long_signals.empty:
        ...
        if long_uses_fallback:
            print(
                color_text(
                    f"  Found {len(long_signals)} LONG signals from ATC (fallback - no oscillator confirmation)",
                    Fore.YELLOW,
                )
            )
        else:
            print(
                color_text(
                    f"  Found {len(long_signals)} confirmed LONG signals (from {original_long_count} ATC signals)",
                    Fore.WHITE,
                )
            )
        print()
        ...
        if has_confidence and not long_uses_fallback:
            print(
                color_text(
                    f"{'Symbol':<15} {'ATC Signal':>12} {'Price':>15} {'Exchange':<10} {'Confidence':>12}", Fore.MAGENTA
                )
            )
        else:
    ...
    if short_signals.empty:
        ...
        if short_uses_fallback:
            print(
                color_text(
                    f"  Found {len(short_signals)} SHORT signals from ATC (fallback - no oscillator confirmation)",
                    Fore.YELLOW,
                )
            )
        else:
            print(
                color_text(
                    f"  Found {len(short_signals)} confirmed SHORT signals (from {original_short_count} ATC signals)",
                    Fore.WHITE,
                )
            )
        print()
    ...
    if (original_long_count + original_short_count) > 0:
        ...
        print(
            color_text(
                f"  Confirmation Rate: {confirmation_rate:.1f}% ({total_confirmed} confirmed by both ATC + Oscillator)",
                Fore.YELLOW,
            )
        )
        if long_uses_fallback or short_uses_fallback:
            fallback_count = (len(long_signals) if long_uses_fallback else 0) + (
                len(short_signals) if short_uses_fallback else 0
            )
            print(
                color_text(
                    f"  Fallback Signals: {fallback_count} signals using ATC only (no oscillator confirmation)",
                    Fore.YELLOW,
                )
            )
    else:
        print(color_text("  Confirmation Rate: N/A", Fore.YELLOW))

    # Display confidence scores
    if avg_long_confidence > 0 or avg_short_confidence > 0:
        print(color_text("  Average Confidence Score:", Fore.WHITE, Style.BRIGHT))
