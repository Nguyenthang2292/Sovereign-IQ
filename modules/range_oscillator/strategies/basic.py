...
def generate_signals_basic_strategy(
    ...
    *,
    ...
    if debug_enabled:
        log_analysis("[Strategy1] Starting signal generation")
        log_debug(
            f"[Strategy1] Parameters: threshold={oscillator_threshold}, "
    ...
    if debug_enabled:
        log_debug(f"[Strategy1] Data shape: oscillator={len(oscillator)}, ma={len(ma)}, range_atr={len(range_atr)}")
    ...
    if debug_enabled:
        ...
        log_debug(f"[Strategy1] Initial signals: LONG={long_count}, SHORT={short_count}, conflicts={conflict_count}")
    ...
    if debug_enabled:
        ...
        log_analysis(
            f"[Strategy1] Final signals: LONG={final_long}, SHORT={final_short}, "
