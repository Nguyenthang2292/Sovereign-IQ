...
def generate_signals_divergence_strategy(
    ...
    *,
    ...
    if debug_enabled:
        log_analysis("[Strategy7] Starting divergence detection signal generation")
        log_debug(
            f"[Strategy7] Parameters: lookback_period={lookback_period}, "
    ...
    osc_trough_values_series = pd.Series(oscillator.values, index=oscillator.index).where(
        is_osc_trough_candidate, np.nan
    )
    ...
    price_peaks_df = (
        ...
        if len(price_peak_indices) > 0
        else pd.DataFrame(columns=["value", "position"])
    )
    ...
    if debug_enabled:
        ...
        log_analysis(
            f"[Strategy7] Final signals: LONG={final_long}, SHORT={final_short}, "
