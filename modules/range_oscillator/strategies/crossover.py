...
def generate_signals_crossover_strategy(
    ...
    *,
    ...
    if debug_enabled:
        log_analysis("[Strategy3] Starting crossover signal generation")
        log_debug(f"[Strategy3] Parameters: confirmation_bars={confirmation_bars}, threshold={oscillator_threshold}")
    ...
    above_confirmation_mask = (
        ...
        & cross_above_shifted.astype(bool).fillna(
            False
        )  # Crossover occurred confirmation_bars ago (fill NaN for beginning)
    )
    ...
    below_confirmation_mask = (
        ...
        & cross_below_shifted.astype(bool).fillna(
            False
        )  # Crossover occurred confirmation_bars ago (fill NaN for beginning)
    )
    ...
    if debug_enabled:
        ...
        log_debug(
            f"[Strategy3] Crossovers: above={cross_above_count} (confirmed={confirmed_above}), "
    ...
    if debug_enabled:
        ...
        log_analysis(
            f"[Strategy3] Final signals: LONG={final_long}, SHORT={final_short}, "
