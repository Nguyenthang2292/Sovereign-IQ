...
from modules.common.utils import log_analysis, log_debug, log_warn
...
class CombinedStrategy:
    ...
    def __init__(self, config: Optional[CombinedStrategyConfig] = None, **kwargs):
        ...
        self.debug_enabled = (
            self.config.enable_debug or os.environ.get("RANGE_OSCILLATOR_DEBUG", "false").lower() == "true"
        )
    ...
    def _build_config(self, config: Optional[CombinedStrategyConfig], **kwargs) -> CombinedStrategyConfig:
        ...
        if kwargs:
            # Consensus
            if "consensus_mode" in kwargs:
                config.consensus.mode = kwargs["consensus_mode"]
            if "consensus_threshold" in kwargs:
                config.consensus.threshold = kwargs["consensus_threshold"]
            if "enable_adaptive_weights" in kwargs:
                config.consensus.adaptive_weights = kwargs["enable_adaptive_weights"]
            if "adaptive_performance_window" in kwargs:
                config.consensus.performance_window = kwargs["adaptive_performance_window"]
            if "weighted_min_diff" in kwargs:
                config.consensus.weighted_min_diff = kwargs["weighted_min_diff"]
            if "weighted_min_total" in kwargs:
                config.consensus.weighted_min_total = kwargs["weighted_min_total"]
            ...
            if "dynamic_selection_lookback" in kwargs:
                config.dynamic.lookback = kwargs["dynamic_selection_lookback"]
            if "dynamic_volatility_threshold" in kwargs:
            ...
            p.breakout_use_dynamic_exhaustion = get_arg(
                "breakout_use_dynamic_exhaustion", p.breakout_use_dynamic_exhaustion
            )
            p.breakout_exhaustion_atr_multiplier = get_arg(
                "breakout_exhaustion_atr_multiplier", p.breakout_exhaustion_atr_multiplier
            )
            p.breakout_base_exhaustion_threshold = get_arg(
                "breakout_base_exhaustion_threshold", p.breakout_base_exhaustion_threshold
            )
            p.breakout_exhaustion_atr_period = get_arg(
                "breakout_exhaustion_atr_period", p.breakout_exhaustion_atr_period
            )
            ...
            p.mean_reversion_extreme_threshold = get_arg(
                "mean_reversion_extreme_threshold", p.mean_reversion_extreme_threshold
            )
            p.mean_reversion_zero_cross_threshold = get_arg(
                "mean_reversion_zero_cross_threshold", p.mean_reversion_zero_cross_threshold
            )
            ...
            if "strategy_weights" in kwargs:
                config.strategy_weights = kwargs["strategy_weights"]
        ...
        if not config.enabled_strategies and "enabled_strategies" not in kwargs:
            es = []
            if config.params.use_sustained:
                es.append(2)
            if config.params.use_crossover:
                es.append(3)
            if config.params.use_momentum:
                es.append(4)
            if config.params.use_breakout:
                es.append(6)
            if config.params.use_divergence:
                es.append(7)
            if config.params.use_trend_following:
                es.append(8)
            if config.params.use_mean_reversion:
                es.append(9)
    ...
    def _validate_config(self) -> None:
        ...
        if c.consensus.mode == "weighted":
            if c.consensus.weighted_min_total < 0:
                raise ValueError("weighted_min_total must be >= 0")
            if c.consensus.weighted_min_diff < 0:
                raise ValueError("weighted_min_diff must be >= 0")
        ...
        if p.use_sustained and p.min_bars_sustained <= 0:
            raise ValueError("min_bars_sustained must be > 0")
        ...
        if p.use_trend_following and p.trend_filter_period <= 0:
            raise ValueError("trend_filter_period must be > 0")

    def _analyze_market_conditions(
        self, oscillator: pd.Series, range_atr: pd.Series, lookback_period: int
    ) -> Dict[str, float]:
        """Analyze market conditions."""
        # DEBUG POINT: Market conditions analysis entry
        # Check: oscillator_len, range_atr_len, lookback_period, notna_count
        ...
        # DEBUG POINT: Market conditions analysis result
        # Check: volatility, trend_strength, range_bound_score, oscillator_extreme, atr_std, atr_mean
    ...
    def _calculate_adaptive_weights(
        ...
        # DEBUG POINT: Adaptive weights calculation entry
        # Check: n_bars, window, enabled_strategies, signals_df_shape, strengths_df_shape
        ...
        for sid in enabled_strategies:
            col = str(sid)
            if col not in recent_signals.columns:
                continue
            ...
            # consensus is a Series, so check if it has any non-zero values for meaningful agreement
            agreement = matches.mean() if len(matches) > 0 else 0.5
        ...
        # DEBUG POINT: Adaptive weights calculation result
        # Check: strategy_scores, total, n_strategies
        if total > 0:
            strategy_scores = {k: v / total * len(enabled_strategies) for k, v in strategy_scores.items()}
        elif not strategy_scores:
            # Fallback: if no scores calculated (e.g., no matching columns), return equal weights
            return {s: 1.0 for s in enabled_strategies}
    ...
    def generate_signals(
        ...
        *,
        ...
        if self.config.dynamic.enabled:
            conditions = self._analyze_market_conditions(oscillator, range_atr, self.config.dynamic.lookback)
            current_enabled_strategies = self._select_strategies_dynamically(conditions, self.config.enabled_strategies)
            if self.debug_enabled:
                log_debug(
                    f"[Strategy5] Dynamic selection: {self.config.enabled_strategies} -> {current_enabled_strategies}"
                )

        # Fallback if no strategies
        if not current_enabled_strategies:
            if self.debug_enabled:
                log_debug("[Strategy5] No strategies enabled, fallback to Strategy 1")
        ...
        run_st(
            ...
            ma=ma,
            range_atr=range_atr,
            min_bars_above_zero=p.min_bars_sustained,
            min_bars_below_zero=p.min_bars_sustained,
            enable_debug=False,
        )
        ...
        run_st(
            ...
            length=length,
            mult=mult,
            upper_threshold=p.breakout_upper_threshold,
            lower_threshold=p.breakout_lower_threshold,
            confirmation_bars=p.breakout_confirmation_bars,
        ...
        if high is not None:
            run_st(
                ...
                length=length,
                mult=mult,
                lookback_period=p.divergence_lookback_period,
                min_swing_bars=p.divergence_min_swing_bars,
                enable_debug=False,
            )
        ...
        run_st(
            ...
            oscillator_threshold=p.trend_oscillator_threshold,
            enable_debug=False,
        )
        ...
        run_st(
            ...
            length=length,
            mult=mult,
            extreme_threshold=p.mean_reversion_extreme_threshold,
            zero_cross_threshold=p.mean_reversion_zero_cross_threshold,
            enable_debug=False,
        )

        if not signals_dict:
            if self.debug_enabled:
                log_debug("[Strategy5] All strategies returned no signals.")
            return self._fallback_strategy1(high, low, close, oscillator, ma, range_atr, length, mult, index)

        # 4. Aggregation
        # DEBUG POINT: Before aggregation - Check signals_dict keys, lengths, index alignment
        # Check: signals_dict_keys, strengths_dict_keys, index_len, signals_dict_lens, index_type

        signals_df = pd.concat(signals_dict, axis=1).fillna(0).astype(int)
        strengths_df = pd.concat(strengths_dict, axis=1).fillna(0.0)

        # DEBUG POINT: After concat, before reindex - Check DataFrame shapes and index alignment
        # Check: signals_df_shape, strengths_df_shape, signals_df_index_equals_target

        signals_df = signals_df.reindex(index, fill_value=0)
        strengths_df = strengths_df.reindex(index, fill_value=0.0)

        # DEBUG POINT: After reindex - Check final DataFrame shapes and empty status
        # Check: signals_df_shape, strengths_df_shape, signals_df_empty, strengths_df_empty, index_equals_target
        ...
        # DEBUG POINT: Before consensus voting - Check arrays and strategy counts
        # Check: signals_array_shape, strengths_array_shape, successful_strategies, n_strategies
        # Check: signals_array_nan_count, strengths_array_nan_count, consensus_mode, consensus_threshold
        ...
        # Calculate Weights
        weights_map = (
            self.config.strategy_weights.copy()
            if self.config.strategy_weights
            else {s: 1.0 for s in successful_strategies}
        )

        if self.config.consensus.adaptive_weights and self.config.consensus.mode == "weighted":
            adaptive_weights = self._calculate_adaptive_weights(signals_df, strengths_df, successful_strategies)
            # Ensure adaptive weights are not empty (fallback to equal weights if empty)
            if adaptive_weights:
                weights_map = adaptive_weights
            else:
                if self.debug_enabled:
                    log_warn("[Strategy5] Adaptive weights returned empty, using equal weights")
        ...
        if self.config.consensus.mode == "weighted":
            ...
            final_signals = np.where(
                (long_votes >= min_agree) & (long_votes > short_votes),
                1,
                np.where((short_votes >= min_agree) & (short_votes > long_votes), -1, 0),
            )

        # 6. Signal Strength
        # Calculate final strengths based on agreeing strategies
        # DEBUG POINT: Before strength calculation - Check final signals distribution
        # Check: final_signals_len, final_signals_long_count, final_signals_short_count, final_signals_neutral_count
        # Check: signals_array_shape, strengths_array_shape
        ...
        # DEBUG POINT: Strength calculation - Check division by zero protection
        # Check: str_sum_len, cnt_sum_len, cnt_sum_zero_count, cnt_sum_max, cnt_sum_min
