...
class BatchChartGenerator:
    ...
    def __init__(
        ...
        """
        Initialize BatchChartGenerator.
        ...
        # BatchChartGenerator plots directly on axes instead
    ...
    def create_batch_chart(
        ...
        if len(symbols_data) > self.charts_per_batch:
            log_error(f"Too many symbols ({len(symbols_data)}), max is {self.charts_per_batch}")
