...
from modules.common.ui.logging import log_debug, log_info, log_error, log_success, log_warn
...
class MultiTimeframeCoordinator:
    ...
    def analyze_deep(
        ...
        **IMPORTANT LIMITATION**: This mode does NOT automatically parse signals from Gemini's
        ...
        log_warn("Deep analysis mode: Signals will be 'NONE' by default. Full analysis text is available for manual review.")
    ...
    def analyze_batch(
        ...
        try:
            ...
            logger.error(
                ...
                symbols, normalized_tfs, e
            )
            # Return empty batch_results structure for all symbols
            return {
                symbol: self._create_empty_symbol_result()
                for symbol in symbols
            }
        except ValueError as e:
            logger.error(
            ...
            logger.exception(
                "Unexpected error generating batch chart for symbols %s across timeframes %s",
                symbols, normalized_tfs
            )
            # Return empty batch_results structure for all symbols
            return {
                symbol: self._create_empty_symbol_result()
        ...
        try:
            ...
            logger.exception(
                "Unexpected error analyzing batch chart for symbols %s across timeframes %s",
                symbols, normalized_tfs
            )
            # Return empty batch_results structure for all symbols
            batch_results = {
