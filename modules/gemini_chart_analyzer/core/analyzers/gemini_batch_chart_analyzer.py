...
class GeminiBatchChartAnalyzer(GeminiChartAnalyzer):
    ...
    def analyze_multi_tf_batch_chart(
        ...
        log_info(f"Analyzing multi-TF batch chart with {len(symbols)} symbols across {len(normalized_timeframes)} timeframes...")
        ...
        try:
            ...
            parsed_results = self._parse_multi_tf_json_response(
                ...
                expected_timeframes=normalized_timeframes
            )
...
"""
    ...
    def _parse_multi_tf_json_response(
        ...
        try:
            ...
            # Log missing symbols if any
            if missing_symbols:
                log_warn(f"{len(missing_symbols)} symbols not found in multi-TF JSON response: {missing_symbols[:5]}{'...' if len(missing_symbols) > 5 else ''}")
    ...
    def _create_empty_multi_tf_result(
        ...
        return {
            symbol: self._create_empty_symbol_multi_tf_result(expected_timeframes)
            for symbol in expected_symbols
        }
    
    def _create_empty_symbol_multi_tf_result(
        self, 
        expected_timeframes: List[str]
    ) -> Dict[str, Any]:
