"""
Legacy Voting Pre-filter logic.

Moved from cli/pre_filter.py to core/prefilter/legacy_voting.py to separate concerns
and resolve naming collisions.
"""
...
def _extract_valid_symbols(df: pd.DataFrame, symbol_col: str, all_symbols_set: Set[str]) -> Set[str]:
    ...
    valid_symbols = symbols[symbols.notna() & (symbols != "") & symbols.isin(all_symbols_set)]
...
def _create_and_setup_analyzer(
    analyzer_class: Union[type[VotingAnalyzer], type[HybridAnalyzer]], args: argparse.Namespace, timeframe: str
...
def pre_filter_symbols_with_hybrid(
    ...
    with _protect_stdin_windows():
        try:
            ...
            filtered_symbols_set = _extract_valid_symbols(analyzer.long_signals_confirmed, "symbol", all_symbols_set)
            filtered_symbols_set.update(
                _extract_valid_symbols(analyzer.short_signals_confirmed, "symbol", all_symbols_set)
            )
