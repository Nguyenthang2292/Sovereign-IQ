...
class ChartBatchGenerator:
    ...
    def __init__(
        ...
        if grid_rows * grid_cols != charts_per_batch:
            raise ValueError(
                f"grid_rows ({grid_rows}) * grid_cols ({grid_cols}) must equal charts_per_batch ({charts_per_batch})"
            )

        # Validate chart_size
        if not isinstance(chart_size, (tuple, list)) or len(chart_size) != 2:
            raise ValueError(
                f"chart_size must be a tuple or list of 2 elements (width, height), got {type(chart_size).__name__}"
            )
    ...
    def _validate_positive_numeric(self, value, param_name: str, allow_types: tuple = (int, float)) -> None:
        """
    ...
    def _plot_simple_chart_on_axes(self, ax: plt.Axes, df: pd.DataFrame, symbol: str, timeframe: str):
        ...
        for timestamp, row in df_candle.iterrows():
            ...
            if body_height > 0:
                ...
                ax.plot(
                    [mdates.date2num(timestamp) - candle_width / 2, mdates.date2num(timestamp) + candle_width / 2],
        ...
        ax.text(
            ...
            bbox=dict(boxstyle="round,pad=0.2", facecolor="black", alpha=0.7, edgecolor="white", linewidth=0.3),
        )
