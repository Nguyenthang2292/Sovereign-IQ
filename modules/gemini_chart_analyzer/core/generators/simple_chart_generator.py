...
FALLBACK_CANDLE_WIDTH = 0.01  # Default candle width in days when insufficient data points
...
class SimpleChartGenerator:
    ...
    def create_simple_chart(
        self, df: pd.DataFrame, symbol: str, timeframe: str, show_symbol_label: bool = True
    ) -> plt.Figure:
        """
        ...
        if len(df_candle) > 1:
            ...
            candle_width = time_diff * CANDLE_WIDTH_RATIO
        else:
            candle_width = FALLBACK_CANDLE_WIDTH
        ...
        with plt.style.context([self.style]):
            ...
            nan_mask = df_candle[["open", "high", "low", "close"]].notna().all(axis=1) & df_candle.index.notna()

            # Count rows to be skipped
            skipped_count = (~nan_mask).sum()
            if skipped_count > 0:
                log_warn(
                    f"Warning: Skipping {skipped_count} row(s) with NaN values in OHLC data for {symbol} ({timeframe})"
                )
            ...
            if len(df_candle_clean) == 0:
                raise ValueError(f"No valid data remaining after filtering NaN values for {symbol} ({timeframe})")
            ...
            for timestamp_num, open_price, high_price, low_price, close_price in zip(
                ...
                if body_height > 0:
                    ...
                    # Doji candle
                    ax.plot(
                        [timestamp_num - candle_width / 2, timestamp_num + candle_width / 2],
                ...
                # Draw wick
                ax.plot([timestamp_num, timestamp_num], [low_price, high_price], color=color, linewidth=0.5, alpha=0.8)
