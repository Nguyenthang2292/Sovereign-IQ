...
class ChartGenerator:
    ...
    def create_chart(
        ...
        with plt.style.context([self.style]):
            ...
            self._plot_separate_indicators(
                indicator_axes, df, indicators, indicator_start_idx=indicator_start_idx, total_rows=total_rows
            )
    ...
    def _create_subplots(self, indicators: Optional[Dict], show_volume: bool) -> Tuple:
        ...
        # Create subplots
        fig, axes = plt.subplots(rows, 1, figsize=self.figsize, gridspec_kw={"height_ratios": height_ratios})
    ...
    def _plot_candlesticks(self, ax, df: pd.DataFrame, symbol: str, timeframe: str, ax_index: int, total_rows: int):
        ...
        rectangles = [
            Rectangle((x_numeric[i] - candle_width / 2, body_lows[i]), candle_width, body_heights[i])
        ...
        if rectangles:
            body_collection = PatchCollection(
                rectangles, facecolors=body_colors, edgecolors=body_colors, linewidths=0.5
            )
        ...
        if "MA" in indicators:
            ...
            for period in periods:
                ...
                if f"MA_{period}" in df.columns:
                    ax.plot(df.index, df[f"MA_{period}"], label=f"MA {period}", color=color, linewidth=1.5, alpha=0.8)
    ...
    def _plot_separate_indicators(
        self, axes, df: pd.DataFrame, indicators: Dict, indicator_start_idx: int, total_rows: int
    ):
        """Plot separate indicators (RSI, MACD, etc.)."""
        ax_idx = 0
