...
class ChartMultiTimeframeBatchGenerator(ChartBatchGenerator):
    ...
    def __init__(
        ...
        """
        Initialize ChartMultiTimeframeBatchGenerator.
        ...
        super().__init__(
            charts_per_batch=total_subplots, grid_rows=total_rows, grid_cols=total_cols, chart_size=chart_size, dpi=dpi
        )
    ...
    def create_multi_tf_batch_chart(
        ...
        if truncated:
            log_warn(f"Too many symbols ({len(symbols_data)}), truncating to {self.symbols_per_batch}")
