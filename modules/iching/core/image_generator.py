...
def _validate_and_sanitize_filename(filename: str) -> str:
    ...
    if not re.match(r"^[a-zA-Z0-9_-]+$", base_name):
        raise ValueError(f"Filename chỉ được chứa letters, numbers, underscore, và hyphen (trước .png): {filename}")

    return filename


def create_hexagram_image(grouped_string: List[str], filename: str = "hexagram.png") -> str:
    """
