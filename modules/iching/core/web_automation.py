...
def scroll_into_view(driver: webdriver.Chrome, element, delay: float = SCROLL_DELAY_DEFAULT) -> None:
...
def click_with_fallback(driver: webdriver.Chrome, element) -> None:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Không thể click element: normal click failed ({e}), JavaScript click failed ({js_e})"
            ) from js_e


def ensure_checkbox_checked(driver: webdriver.Chrome, wait: WebDriverWait, checkbox_id: str) -> None:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Không thể tick checkbox {checkbox_id}: {exc}, JavaScript fallback cũng thất bại: {js_exc}"
            ) from js_exc
...
def click_submit_button(driver: webdriver.Chrome, wait: WebDriverWait) -> None:
    ...
    try:
        submit_button = wait.until(EC.element_to_be_clickable((By.NAME, "submitsearch")))
        ...
        try:
            submit_button = driver.find_element(By.XPATH, "//button[contains(text(), 'Lập quẻ')]")
            scroll_into_view(driver, submit_button, delay=SCROLL_DELAY_LONG)
            click_with_fallback(driver, submit_button)
            time.sleep(SUBMIT_DELAY)
...
def fill_web_form(line_info: List[Dict[str, bool]], auto_close: bool = False, stop_on_error: bool = False) -> None:
    """
