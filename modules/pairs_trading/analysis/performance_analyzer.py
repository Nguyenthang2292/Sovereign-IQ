...
class PerformanceAnalyzer:
    ...
    def __init__(
        ...
        if abs(total_weight - 1.0) > 0.01:
            raise ValueError(f"Weights must sum to 1.0, got {total_weight:.6f}. Current weights: {self.weights}")
        ...
        if min_candles > limit:
            raise ValueError(f"min_candles ({min_candles}) must be <= limit ({limit})")
    ...
    def _check_consecutive_nan_chunks(
        self, values: np.ndarray, symbol: str, max_gap_ratio: float = 0.1, max_gap_candles: int = 24
    ) -> bool:
        """
    ...
    def calculate_performance_score(self, symbol: str, df: pd.DataFrame) -> Optional[Dict[str, float]]:
        """
    ...
    def analyze_all_symbols(
        ...
        # Validate inputs
        if not symbols:
            return pd.DataFrame(columns=["symbol", "score", "1d_return", "3d_return", "1w_return", "current_price"])
    ...
    def get_top_performers(self, df: pd.DataFrame, top_n: int = PAIRS_TRADING_TOP_N) -> pd.DataFrame:
        """
    ...
    def get_worst_performers(self, df: pd.DataFrame, top_n: int = PAIRS_TRADING_TOP_N) -> pd.DataFrame:
        """
