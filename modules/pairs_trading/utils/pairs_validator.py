...
def validate_pairs(
    ...
    for _, row in pairs_df.iterrows():
        ...
        if correlation is not None and not pd.isna(correlation) and not np.isinf(correlation):
            ...
            if abs_corr < min_correlation:
                is_valid = False
                validation_errors.append(f"Correlation too low ({abs_corr:.3f} < {min_correlation:.3f})")
            elif abs_corr > max_correlation:
                is_valid = False
                validation_errors.append(f"Correlation too high ({abs_corr:.3f} > {max_correlation:.3f})")
        ...
        if max_half_life is not None:
            ...
            if half_life is not None and not pd.isna(half_life) and not np.isinf(half_life):
                if half_life > max_half_life:
                    is_valid = False
                    validation_errors.append(f"Half-life too high ({half_life:.1f} > {max_half_life})")
        ...
        if min_spread_sharpe is not None:
            ...
            if spread_sharpe is not None and not pd.isna(spread_sharpe) and not np.isinf(spread_sharpe):
                if spread_sharpe < min_spread_sharpe:
                    is_valid = False
                    validation_errors.append(f"Sharpe ratio too low ({spread_sharpe:.2f} < {min_spread_sharpe})")
    ...
    df_validated = df_validated.sort_values("opportunity_score", ascending=False).reset_index(drop=True)
