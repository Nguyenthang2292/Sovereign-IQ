...
def ensure_symbols_in_candidate_pools(
    performance_df: pd.DataFrame, best_df: pd.DataFrame, worst_df: pd.DataFrame, target_symbols: List[str]
    ...
    best_df = best_df.sort_values("score", ascending=False).reset_index(drop=True)
    worst_df = worst_df.sort_values("score", ascending=True).reset_index(drop=True)
