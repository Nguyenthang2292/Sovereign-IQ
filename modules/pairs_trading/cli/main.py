...
def main() -> None:
    ...
    if strategy == "momentum":
        ...
        if getattr(args, "require_cointegration", False):
            if log_warn:
                log_warn(
                    "Momentum strategy bỏ qua kiểm tra cointegration nghiêm ngặt. Tự động tắt --require-cointegration."
                )
    ...
    try:
        performance_df = performance_analyzer.analyze_all_symbols(symbols, data_fetcher, verbose=True)
    ...
    best_performers = performance_df.sort_values("score", ascending=False).head(candidate_depth).reset_index(drop=True)
    worst_performers = performance_df.sort_values("score", ascending=True).head(candidate_depth).reset_index(drop=True)
    ...
    try:
        ...
        if target_symbols:
            ...
            matched_symbols = {
                ...
                if not manual_pairs[(manual_pairs["long_symbol"] == sym) | (manual_pairs["short_symbol"] == sym)].empty
            }
        ...
        if not selected_pairs.empty:
            ...
            if strategy == "momentum":
                ...
                hurst_values = (
                    selected_pairs.get("hurst_exponent").dropna()
                    if "hurst_exponent" in selected_pairs.columns
                    else pd.Series()
                )
                ...
                zscore_values = (
                    selected_pairs.get("current_zscore").dropna()
                    if "current_zscore" in selected_pairs.columns
                    else pd.Series()
                )
                ...
                # Average half-life
                half_lives = (
                    selected_pairs.get("half_life").dropna() if "half_life" in selected_pairs.columns else pd.Series()
                )
            ...
            sharpe_ratios = (
                selected_pairs.get("spread_sharpe").dropna()
                if "spread_sharpe" in selected_pairs.columns
                else pd.Series()
            )
            ...
            max_dds = (
                selected_pairs.get("max_drawdown").dropna() if "max_drawdown" in selected_pairs.columns else pd.Series()
            )
            ...
            # OLS hedge ratio
            hedge_ratios = (
                selected_pairs.get("hedge_ratio").dropna() if "hedge_ratio" in selected_pairs.columns else pd.Series()
            )
            ...
            # Kalman hedge ratio
            kalman_ratios = (
                selected_pairs.get("kalman_hedge_ratio").dropna()
                if "kalman_hedge_ratio" in selected_pairs.columns
                else pd.Series()
            )
