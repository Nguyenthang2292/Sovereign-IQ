# Adaptive Trend Classification Enhanced (ATC Enhanced)

**Enhanced version with GPU/CPU/RAM optimization and automatic memory management**

Module Adaptive Trend Classification Enhanced là phiên bản cải tiến của ATC với:
...
- **Memory Management**: Automatic cleanup, monitoring và prevention của memory leaks
- **Numba JIT**: JIT compilation cho MA calculations (DEMA, WMA, LSMA)
- **Caching**: Intelligent caching cho MA results
- **NumPy Optimization**: Pre-allocated arrays và NumPy operations thay vì Pandas

Module gốc cung cấp hệ thống phân tích xu hướng thích ứng sử dụng nhiều loại Moving Averages với adaptive weighting dựa trên equity curves.
...
```text
adaptive_trend_enhance/
├── __init__.py              # Module exports
├── README.md                # Tài liệu này
├── core/
│   ├── __init__.py          # Core exports
│   ├── analyzer.py          # Phân tích symbol đơn lẻ (ENHANCED)
│   ├── scanner.py          # Scan nhiều symbols (ENHANCED)
│   ├── compute_atc_signals.py    # Tính toán ATC signals (ENHANCED với GPU/CPU)
│   ├── compute_equity.py         # Tính toán equity curves (ENHANCED)
│   ├── compute_moving_averages.py # Tính toán MA (ENHANCED với Numba JIT + GPU)
│   ├── process_layer1.py         # Xử lý Layer 1 (ENHANCED)
│   ├── signal_detection.py       # Phát hiện signals (ENHANCED)
│   ├── hardware_manager.py       # NEW: CPU/GPU/RAM auto-detection & allocation
│   └── memory_manager.py         # NEW: Memory monitoring & automatic cleanup
├── cli/
│   ├── __init__.py          # CLI exports
│   ├── argument_parser.py   # Parse command-line arguments
│   ├── display.py           # Hiển thị kết quả (với hardware metrics)
│   ├── interactive_prompts.py # Interactive prompts
│   └── main.py              # Main CLI entry point
└── utils/
    ├── __init__.py          # Utils exports
    ├── config.py            # ATCConfig (ENHANCED với hardware settings)
    ├── rate_of_change.py   # Tính toán rate of change (ENHANCED)
    ├── diflen.py            # Tính toán độ dài khác biệt (ENHANCED)
    ├── exp_growth.py        # Exponential growth factor (ENHANCED)
    └── cache_manager.py     # NEW: Intelligent caching for MA results
```
