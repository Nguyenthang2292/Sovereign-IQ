...
///
/// TRUE Hull Moving Average formula:
/// HMA = WMA(2 * WMA(n/2) - WMA(n), sqrt(n))
///
/// This provides better trend following and reduces lag compared to SMA.
///
/// # Arguments
///
/// * `prices_arr` - Array view of price values
...
mod tests {
    ...
    fn test_ema_basic() {
        ...
        assert!(!ema[4].is_nan());
        // EMA uses SMA init: first value at index length-1, earlier indices are NaN
        assert!(ema[0].is_nan());
        assert!((ema[2] - 11.0).abs() < 1e-6); // SMA of first 3
    }
