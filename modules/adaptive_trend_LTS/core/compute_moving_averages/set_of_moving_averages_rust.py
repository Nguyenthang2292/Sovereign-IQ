...
def set_of_moving_averages_rust(
    length: int,
    source: pd.Series,
    ma_type: str,
    ...
    if len(source) == 0:
        log_warn("Empty source series for set_of_moving_averages_rust")
        return None
    ...
    try:
        ...
        if use_parallel:
            ...
            with ThreadPoolExecutor(max_workers=config.num_threads) as executor:
                futures = [
                    executor.submit(ma_calculation_rust, source, ma_len, ma_type, use_cache, use_rust, use_cuda)
                    for ma_len in ma_lengths
                ]
                mas = [f.result() for f in futures]
        else:
            mas = [ma_calculation_rust(source, ma_len, ma_type, use_cache, use_rust, use_cuda) for ma_len in ma_lengths]
...
__all__ = ["set_of_moving_averages_rust"]
