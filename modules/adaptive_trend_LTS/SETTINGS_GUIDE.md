...
1. **Rust** (nếu đã build) ⭐ **FASTEST** - 3.77x speedup
2. **CUDA** (nếu `use_cuda=True`) - 1.94x speedup, 98% accuracy
3. **Numba JIT** (fallback)
4. **Pure Python** (slowest)
...
config = {
    # ... standard params ...
    'use_cuda': False,  # Rust faster than CUDA
    'parallel_l1': True,
    'parallel_l2': True,
    'use_cache': True,
    'fast_mode': True,
...
| **Rust** ⭐ | **13.94s** | **3.77x** | **25.7 MB** | **98%** |
| CUDA Batch | 27.04s | 1.94x | 71.3 MB | 98% |

**Recommendation**: **USE RUST** - Best balance of speed, memory, accuracy
