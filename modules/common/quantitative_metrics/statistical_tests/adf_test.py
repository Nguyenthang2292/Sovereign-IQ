...
This is a general-purpose ADF test that can be used for any time series
...
def calculate_adf_test(
    ...
    """
    Run Augmented Dickey-Fuller (ADF) test to check if time series is stationary.
    
    Tests whether a time series has a unit root (non-stationary). Stationary series
    are mean-reverting and suitable for certain trading strategies.
    
    **Interpretation**:
    - **p-value < 0.05**: Reject H0 → Stationary → Good for mean-reversion strategies
    ...
    Args:
        spread: Time series to test (pd.Series). Missing values auto-removed.
    ...
    Note:
        - p-value < 0.05 (preferably < 0.01) indicates stationarity
