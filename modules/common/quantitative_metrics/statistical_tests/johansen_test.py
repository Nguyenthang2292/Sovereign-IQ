...
"""
Johansen cointegration test for quantitative analysis.

This is a general-purpose cointegration test that can be used for any
multivariate time series analysis, not just pairs trading.
"""
...
def calculate_johansen_test(
    ...
    """
    Run Johansen cointegration test to determine if two time series are cointegrated.
    ...
    and tend to revert to mean.

    Args:
        price1: First time series (pd.Series, should be aligned with price2)
        price2: Second time series (pd.Series, should be aligned with price1)
        ...
        det_order: Deterministic order in VECM model (must be in [-1, 0, 1]).
            0: No constant/trend (spread mean-reverts to zero) - most common
