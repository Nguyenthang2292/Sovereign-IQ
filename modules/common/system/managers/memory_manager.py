"""
Memory Manager for all modules
...
class MemoryManager:
    ...
    def get_current_usage(self) -> MemorySnapshot:
        ...
        snapshot = MemorySnapshot(timestamp=time.time(), ram_used_gb=0.0, ram_percent=0.0, ram_available_gb=0.0)

        # RAM usage using detection layer
        memory_info = SystemInfo.get_memory_info()
        snapshot.ram_used_gb = memory_info.used_gb
        snapshot.ram_percent = memory_info.percent_used
    ...
    @contextmanager
    def safe_memory_operation(self, operation_name: str = "operation", cleanup_after: bool = True):
        """
    ...
    def temp_series_scope(self):
        """
        Context manager to ensure GC after operations that create many temporary series.
        """
        try:
            yield
        finally:
...
def temp_series(func: Callable):
    """
    Decorator to automatically trigger memory cleanup after a function call.
    Useful for functions that generate many intermediate pandas Series.
    """
...
def cleanup_series(*series_list: Union[pd.Series, pd.DataFrame]):
    """
    Explicitly trigger cleanup after processing large Series or DataFrames.

    Args:
        series_list: One or more pandas Series or DataFrames to check for size.
    """
    large_detected = False
    for s in series_list:
        if s is not None and hasattr(s, "memory_usage"):
            try:
                # If any series is > 50MB, trigger more aggressive GC
                usage = s.memory_usage(deep=True)
...
def get_memory_manager(enable_tracemalloc: bool = False) -> MemoryManager:
    ...
    """
    # Note: SingletonMeta doesn't support kwargs, so we need to handle this differently
    # For now, we'll create a new instance if enable_tracemalloc is True and no instance exists
    instance = MemoryManagerSingleton()
...
def track_memory(operation_name: str = "operation"):
    ...
    Example:
        from modules.common.system.managers.memory_manager import track_memory
