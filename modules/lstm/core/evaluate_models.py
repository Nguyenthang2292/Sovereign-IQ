...
from modules.common.ui.logging import log_warn, log_error, log_analysis
...
def _infer_output_classes(
    ...
    X_test: torch.Tensor, 
    device: torch.device, 
    default_classes: int = DEFAULT_NUM_CLASSES
) -> int:
    """
...
def evaluate_model_in_batches(
    ...
    try:
        ...
        if concat_pred.shape[0] != len(X_test):
            log_error(
                f"Concatenated predictions have wrong number of samples: "
                f"{concat_pred.shape[0]} vs X_test {len(X_test)}"
            )
            raise RuntimeError(
