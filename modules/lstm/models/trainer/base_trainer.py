...
class BaseLSTMTrainer:
    ...
    def _setup_device(self) -> None:
        ...
        self.use_mixed_precision = self.device.type == "cuda" and torch.cuda.get_device_capability(0)[0] >= 7
    ...
    def _validate_and_preprocess_data(self, df_input: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(X) == 0:
            ...
            if not basic_cols:
                ...
                raise ValueError(
                    f"Data preprocessing failed - no valid sequences created. Available basic columns: {basic_cols}"
                )
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, int, np.ndarray]:
        """
    ...
    def _setup_training_components(
        ...
        optim.Optimizer, optim.lr_scheduler._LRScheduler, DataLoader, DataLoader, nn.Module, Optional[GradScaler]
    ]:
        """
        ...
        optimizer = optim.AdamW(
            model.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay, eps=self.adam_eps
        )
        scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=self.scheduler_T_0, T_mult=self.scheduler_T_mult, eta_min=self.scheduler_eta_min
        )
    ...
    def _validate_epoch(self, model: nn.Module, val_loader: DataLoader, criterion: nn.Module) -> Tuple[float, int, int]:
        """
        ...
        with torch.no_grad():
            for batch_X, batch_y in val_loader:
                batch_X, batch_y = (
                    ...
                    batch_y.to(self.device, non_blocking=True),
                )
    ...
    def _evaluate_model(
        self, model: nn.Module, X_test: torch.Tensor, y_test: torch.Tensor, df_input: pd.DataFrame
    ) -> None:
        """
        ...
        if self.output_mode == "regression":
            ...
            log_model(
                f"Regression optimization - Threshold: {best_threshold or 0.02:.4f}, "
            ...
            log_model(
                f"Classification optimization - Confidence: {best_confidence or 0.7:.2f}, "
                f"Sharpe: {best_sharpe if best_sharpe is not None else 0.0:.4f}"
            )
