...
class LSTMTrainer(BaseLSTMTrainer, CNNFeatureMixin):
    ...
    def create_model(self, input_size: int, **kwargs) -> nn.Module:
        ...
        log_model(
            f"{model_type} model created - Input: {input_size}, Look back: {self.look_back}, Classes: {num_classes}"
        )
        return model
    ...
    def _setup_training_components(
        self, input_size: int, X_train: torch.Tensor, y_train: torch.Tensor, X_val: torch.Tensor, y_val: torch.Tensor
    ) -> Tuple[
        nn.Module,
        optim.Optimizer,
    ...
    def train(
        ...
        try:
            ...
            model, optimizer, scheduler, train_loader, val_loader, criterion, scaler_amp = (
                self._setup_training_components(input_size, X_train, y_train, X_val, y_val)
            )
            ...
            model_type = self._get_model_type_name()
            log_model(
                f"Training {model_type} for {epochs} epochs - Batch size: {train_loader.batch_size}, Mixed precision: {self.use_mixed_precision}"
            )
