...
from modules.lstm.core import CNN1DExtractor, FeedForward, MultiHeadAttention, PositionalEncoding
...
class LSTMAttentionModel(nn.Module):
    ...
    def __init__(
        ...
        self.multihead_attention = MultiHeadAttention(d_model=self.attention_dim, num_heads=num_heads, dropout=dropout)

        self.feed_forward = FeedForward(d_model=self.attention_dim, d_ff=self.attention_dim * 2, dropout=dropout)
...
class CNNLSTMAttentionModel(nn.Module):
    ...
    def __init__(
        ...
        self.cnn_extractor = CNN1DExtractor(input_channels=input_size, cnn_features=cnn_features, dropout=dropout)
        ...
        if use_attention:
            ...
            self.multihead_attention = MultiHeadAttention(
                d_model=self.attention_dim, num_heads=num_heads, dropout=dropout
            )
