...
def get_latest_lstm_attention_signal(
    df_market_data: pd.DataFrame, model: nn.Module, scaler: Optional[MinMaxScaler] = None
) -> str:
    """
...
def load_model_and_scaler(
...
) -> tuple[Optional[nn.Module], Optional[MinMaxScaler], Optional[int]]:
    """
    ...
    try:
        ...
        if scaler is None:
            ...
            log_info("Loaded scaler from checkpoint")

        if use_kalman_filter:
            log_info("Model was trained with Kalman Filter preprocessing")
