...
from modules.common.ui.logging import log_model, log_warn, log_debug
    ...
    **Important Limitations:**
    ...
    try:
        if device.type == 'cpu':
            ...
            log_model(f"CPU: Model: {model_type}, Mode: {mode_str}, Estimated per-sample MB: {memory_per_sample_mb:.4f}, Optimal batch: {optimal_batch}")
            log_warn("Memory estimation is approximate and may underestimate actual requirements. "
            ...
            if memory_per_sample_mb > 0:
                ...
                log_warn(f"Memory per sample is zero (input_size={input_size}, sequence_length={sequence_length}), using default GPU_BATCH_SIZE")
            ...
            if estimated_memory_gb > usable_memory_gb:
                ...
                max_possible_batch = int((usable_memory_gb * 1024) // memory_per_sample_mb) if memory_per_sample_mb > 0 else MIN_BATCH_SIZE
                optimal_batch = max(MIN_BATCH_SIZE, min(optimal_batch, max_possible_batch))
                estimated_memory_gb = (optimal_batch * memory_per_sample_mb) / 1024
                if estimated_memory_gb > usable_memory_gb:
                    log_warn(f"Even the reduced batch size ({optimal_batch}) may exceed available memory ({usable_memory_gb:.2f}GB)")

            mode_str = "training" if is_training else "inference"
            log_model("GPU Memory: {0:.1f}GB, Model: {1}, Mode: {2}, Optimal batch size: {3}".format(
                gpu_memory_gb, model_type, mode_str, optimal_batch))
            log_debug("Estimated memory usage: {0:.2f}GB ({1:.1f}%)".format(
        ...
        fallback_sizes = {
            ...
            'lstm_attention': max(FALLBACK_MIN_BATCH_LSTM_ATTENTION, GPU_BATCH_SIZE // FALLBACK_BATCH_DIVISOR_LSTM_ATTENTION),
            'lstm': GPU_BATCH_SIZE
        }
