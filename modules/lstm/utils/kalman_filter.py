...
class KalmanFilterOHLC:
    ...
    def _kalman_step(self, observation: float, prev_state: float, prev_uncertainty: float) -> tuple[float, float]:
        """
    ...
    def fit(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in required_cols:
            ...
            for i in range(initial_idx, len(series)):
                obs = series[i]
                current_state, current_uncertainty = self._kalman_step(obs, current_state, current_uncertainty)
        ...
        self.fitted = True
        log_model(
            f"Kalman Filter applied to OHLC data: process_var={self.process_variance:.2e}, "
...
def apply_kalman_to_ohlc(
    df: pd.DataFrame, process_variance: float = 1e-5, observation_variance: float = 1.0, **kwargs
) -> pd.DataFrame:
    """
    ...
    try:
        kf = KalmanFilterOHLC(process_variance=process_variance, observation_variance=observation_variance, **kwargs)
        return kf.fit(df)
