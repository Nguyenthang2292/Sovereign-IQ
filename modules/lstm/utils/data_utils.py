...
from config import TRAIN_TEST_SPLIT, VALIDATION_SPLIT
...
def split_train_test_data(
    ...
    X_temp, X_test, y_temp, y_test, _, test_indices = train_test_split(
        X, y, indices, test_size=test_ratio, shuffle=shuffle, random_state=random_state
    )
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X_temp, y_temp, test_size=val_size_relative, shuffle=shuffle, random_state=random_state
    )

    log_model(f"Data split - Train: {len(X_train)}, Val: {len(X_val)}, Test: {len(X_test)}")
    if shuffle and return_indices:
        log_model(
            "WARNING: shuffle=True detected. For time series data, ensure test_indices are used for price alignment."
        )
