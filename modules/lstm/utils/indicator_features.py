...
from modules.common.ui.logging import log_model, log_warn, log_error
...
from config.model_features import MODEL_FEATURES, CANDLESTICK_PATTERN_NAMES
...
def generate_indicator_features(df_input: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if candlestick_features:
            log_warn("Candlestick patterns are not calculated in this version. Setting all candlestick pattern features to 0.0.")
        ...
        if rows_after > 0 and rows_after < rows_before * 0.5:
            log_warn(f"Significant data loss: {rows_before - rows_after} of {rows_before} rows dropped due to NaN values ({(1 - rows_after/rows_before)*100:.1f}%)")
        ...
        if result_df.empty:
            ...
            log_model(f"Technical indicators calculated successfully: {len(calculated_features)} features for {len(result_df)} rows")
