...
from modules.common.ui.logging import (
    log_info, log_success, log_error, log_warn, log_debug,
    log_model, log_analysis, log_system
)
...
try:
    ...
    log_success(f"PyTorch {torch.__version__} loaded successfully")
    ...
    if torch.cuda.is_available():
        try:
            ...
            log_system(f"CUDA {cuda_version} available with {torch.cuda.device_count()} device(s)")
        except Exception as cuda_error:
            log_warn(f"CUDA available but not functional: {cuda_error}")
            log_warn("Falling back to CPU mode...")
            torch.cuda.is_available = lambda: False  
    else:
        log_info("CUDA not available, using CPU mode")
        
except ImportError as e:
    log_error(f"Failed to import PyTorch: {e}")
    log_error("Please reinstall PyTorch: pip install torch torchvision torchaudio")
    sys.exit(1)
...
def preprocess_cnn_lstm_data(df_input: pd.DataFrame, look_back: int = WINDOW_SIZE_LSTM, output_mode: str = 'classification', scaler_type: str = 'minmax') -> Tuple[np.ndarray, np.ndarray, Union[MinMaxScaler, StandardScaler], list[str]]:
    ...
    if len(df) < look_back + 1:
        log_error(f"Insufficient data after cleanup: {len(df)} rows (dropped {initial_len - len(df)} NaN)")
        return np.array([]), np.array([]), MinMaxScaler(), []
    ...
    if np.isnan(features).any() or np.isinf(features).any():
        log_warn("Cleaning invalid values in features")
...
def split_train_test_data(X: np.ndarray, y: np.ndarray, train_ratio: float = TRAIN_TEST_SPLIT, validation_ratio: float = VALIDATION_SPLIT) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    log_model(f"Data split - Train: {train_end}, Val: {val_end - train_end}, Test: {n_samples - val_end}")
...
def create_cnn_lstm_attention_model(input_size: int, use_attention: bool = True, 
    ...
    if use_cnn:
        ...
        log_model("Created CNN-LSTM-Attention model with {0} mode".format(output_mode))
        return model
    
    if use_attention:
        log_model("Creating LSTM model with Multi-Head Attention")
    ...
    log_model("Creating standard LSTM model")
...
def train_cnn_lstm_attention_model(df_input: pd.DataFrame,
    ...
    if use_mixed_precision:
        ...
        log_system("Using mixed precision training for CNN-LSTM")
    ...
    log_model(f"Starting CNN-LSTM pipeline - Look back: {look_back}, Mode: {output_mode}, CNN: {use_cnn}")
    ...
    if len(X) == 0:
        ...
        log_error(f"Feature names returned: {feature_names}")
        ...
        if not basic_cols:
            ...
            if 'close' in df_input.columns or COL_CLOSE in df_input.columns:
                ...
                if len(X) > 0:
                    log_warn(f"Successfully created {len(X)} sequences with minimal features")
                else:
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = split_train_test_data(X, y, TRAIN_TEST_SPLIT, VALIDATION_SPLIT)
    ...
    try:
        ...
        log_model(f"{model_type} model created - Input: {input_size}, Look back: {look_back}, Classes: {num_classes}")
    ...
    if use_cnn:
        ...
        log_system(f"CNN optimized batch size: {optimal_batch_size}")
    ...
    log_model(f"Training {model_type} for {epochs} epochs - Batch size: {optimal_batch_size}, Mixed precision: {use_mixed_precision}")
        ...
        for epoch in range(epochs):
            ...
            if use_early_stopping:
                if val_loss < best_val_loss:
                    ...
                    log_debug(f"New best validation loss: {val_loss:.4f}")
                else:
                    patience_counter += 1
    ...
    if save_model and model is not None:
        try:
            ...
            log_success(f"CNN-LSTM model saved to {model_path}")
...
def train_and_save_global_cnn_lstm_attention_model(combined_df: pd.DataFrame, 
    ...
    try:
        ...
        if gpu_available:
            ...
            log_system("Using GPU for CNN-LSTM model training")
        else:
            log_info("Using CPU for CNN-LSTM model training")
        ...
        log_model(f"Training CNN-LSTM{'-Attention' if use_attention else ''} model...")
        ...
        if not model:
            log_error("Model training failed - returned None")
            return None, ""

        log_model(f"Saving model to: {model_path}")
        torch.save({
            'model_state_dict': model.state_dict(),
            'model_config': {
        ...
        log_analysis(f"CNN-LSTM model trained and saved in {elapsed_time:.2f}s: {model_path}")

        return model, str(model_path)

    except Exception as e:
        log_error(f"Error during CNN-LSTM model training: {e}")
        log_error(f"Traceback: {traceback.format_exc()}")
        return None, ""