...
class HMMSignalCombiner:
    ...
    def combine(self, df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        """
        ...
        for strategy in strategies:
            try:
                ...
                results[strategy.name] = HMMStrategyResult(
                    signal=HOLD, probability=0.0, state=0, metadata={"error": error_msg}
                )
        ...
        # Normalize scores
        normalized_long, normalized_short = normalize_strategy_scores(score_long, score_short, strategies, results)
        ...
        # Combined confidence
        combined_confidence = calculate_combined_confidence(avg_probability, kama_confidence, signal_agreement)
        ...
        if HMM_FEATURES.get("normalization_enabled", True):
            ...
            base_threshold = (
                (HMM_SIGNAL_MIN_THRESHOLD / max_possible * 100) if max_possible > 0 else HMM_SIGNAL_MIN_THRESHOLD
            )
        else:
    ...
    def _log_results(
        ...
        for name, signal in signals.items():
            ...
            if result:
                signal_info_parts.append(
                    f"{name}: {signal_map[signal]} (state: {result.state}, prob: {result.probability:.3f})"
                )
        ...
        conflict_info = ""
        if HMM_FEATURES.get("conflict_resolution_enabled", True):
            conflicts = [name for name in signals.keys() if original_signals.get(name) != signals.get(name)]
            if conflicts:
...
def combine_signals(df: pd.DataFrame, registry: Optional[HMMStrategyRegistry] = None, **kwargs) -> Dict[str, Any]:
    """
