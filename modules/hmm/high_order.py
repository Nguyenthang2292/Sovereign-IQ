...
def convert_swing_to_state(swing_highs_info: pd.DataFrame, swing_lows_info: pd.DataFrame, strict_mode: bool = False) -> List[float]:
    ...
    if strict_mode:
        ...
        if min_length < len(swing_highs_info) or min_length < len(swing_lows_info):
            log_warn(f"Trimming data - using {min_length} points (highs: {len(swing_highs_info)}, lows: {len(swing_lows_info)})")
...
def hmm_high_order(
    ...
    # Find swing points
    orders_argrelextrema_val = (
        orders_argrelextrema
        if orders_argrelextrema is not None
        else HMM_HIGH_ORDER_ORDERS_ARGRELEXTREMA_DEFAULT
    )
    swing_highs = argrelextrema(df['high'].values, np.greater, order=orders_argrelextrema_val)[0]
    swing_lows = argrelextrema(df['low'].values, np.less, order=orders_argrelextrema_val)[0]
    ...
    states = convert_swing_to_state(swing_highs_info, swing_lows_info, strict_mode=strict_mode_val)
    if not states:
