...
def predict_next_hidden_state_forward_backward_high_order(
    model: DenseHMM, observations: list, order: int = 1, n_base_states: int = N_BASE_STATES
) -> List[float]:
    """
...
def predict_next_observation_high_order(
    ...
    """
    next_hidden_proba = predict_next_hidden_state_forward_backward_high_order(model, observations, order, n_base_states)
