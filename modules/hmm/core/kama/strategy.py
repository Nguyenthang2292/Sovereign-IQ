...
class KamaHMMStrategy(HMMStrategy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.window_kama = window_kama if window_kama is not None else HMM_WINDOW_KAMA_DEFAULT
        self.fast_kama = fast_kama if fast_kama is not None else HMM_FAST_KAMA_DEFAULT
        self.slow_kama = slow_kama if slow_kama is not None else HMM_SLOW_KAMA_DEFAULT
        self.window_size = window_size if window_size is not None else HMM_WINDOW_SIZE_DEFAULT
    ...
    def analyze(self, df: pd.DataFrame, **kwargs) -> "HMMStrategyResult":
        ...
        return HMMStrategyResult(signal=signal, probability=probability, state=primary_state, metadata=metadata)
