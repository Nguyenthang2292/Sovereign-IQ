...
class CorrelationAnalyzer:
    ...
    def calculate_weighted_correlation(self, new_symbol: str, verbose: bool = True):
        ...
        for pos in self.positions:
            pos_df, _ = self.data_fetcher.fetch_ohlcv_with_fallback_exchange(pos.symbol)
            pos_series = self.data_fetcher.dataframe_to_close_series(pos_df)
    ...
    def calculate_portfolio_return_correlation(
        ...
        for pos in self.positions:
            if pos.symbol not in symbol_series:
                df, _ = self.data_fetcher.fetch_ohlcv_with_fallback_exchange(pos.symbol)
