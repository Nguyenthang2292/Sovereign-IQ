...
def calculate_signals_incremental(
    ...
    try:
        ...
        log_progress(f"  Indicators pre-computed successfully. Starting incremental calculation with position-aware skipping...")
    ...
    try:
        for i in range(len(df)):
            ...
            # Skip signal calculation if position is open
            if position is not None:
                signals.iloc[i] = 0  # No new signal when position open
                progress.update()
                continue
