...
def calculate_signals(
    ...
    log_progress("  Calculating signals from pre-computed indicators...")
    print()  # Newline before progress bar
    ...
    log_progress(
        f"  Cache stats: {cache_stats['signal_cache_size']}/{cache_stats['signal_cache_max_size']} signals cached"
    )
...
def calculate_single_signals(
    ...
    # STEP 2: Calculate signals incrementally from pre-computed indicators
    log_progress("  Calculating signals from pre-computed indicators (single signal mode)...")
    print()  # Newline before progress bar
...
def calculate_single_signals_parallel(
    ...
    log_progress(
        f"  Calculating signals in parallel (single signal mode): {len(batches)} batches, {num_workers} workers"
    )
    print()  # Newline before progress bar
