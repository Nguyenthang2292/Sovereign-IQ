...
def draw_ChoCh(fig: go.Figure, df: pd.DataFrame, state: SMCState, internal: bool = True) -> go.Figure:
    ...
    if internal:
        bullish_times = state.internal_structure.bullish_choch.get("Pivot_bullishChoch_Time", pd.Series(dtype="datetime64[ns]"))
        bearish_times = state.internal_structure.bearish_choch.get("Pivot_bearishChoch_Time", pd.Series(dtype="datetime64[ns]"))
    ...
    if trend_direction == BULLISH:
        if internal:
            ...
            if bullish_swing_x:
                fig.add_trace(go.Scatter(
                    x=bullish_swing_x, y=bullish_swing_y,
                    mode='markers+text',
                    marker=dict(color='green', size=10, symbol='triangle-up'),
        ...
        if internal:
            ...
            if bearish_swing_x:
                fig.add_trace(go.Scatter(
                    x=bearish_swing_x, y=bearish_swing_y,
                    mode='markers+text',
                    marker=dict(color='green', size=10, symbol='triangle-up'),
