        ...
        for future in as_completed(future_to_timeframe):
            ...
            try:
                ...
                if result_timeframe is not None and result_df is not None:
                    ...
                    # Set cancellation event so running tasks can stop early
                    cancel_event.set()
...
def render_configuration_summary(
    ...
    if signal_mode == 'single_signal':
        print(f"    └─ Using single signal (highest confidence) approach - Use any signal with highest confidence")
    else:
        print(f"    └─ Using majority vote approach - Requires at least {color_text('3 indicators', Fore.CYAN)} to agree")
