  ...
  <div class="workflow-diagrams max-w-7xl mx-auto px-6 py-6">
    ...
    <div v-if="activeTab === 'voting'" class="space-y-6">
      <div class="glass-panel rounded-lg p-6">
        ...
        <!-- Module Details -->
        <ModuleDetails 
          ref="votingModuleDetails"
          :modules="votingModules" 
          :highlighted-module="highlightedModule"
        />
      </div>
    </div>
    ...
    <div v-if="activeTab === 'hybrid'" class="space-y-6">
      <div class="glass-panel rounded-lg p-6">
        ...
        <!-- Module Details -->
        <ModuleDetails 
          ref="hybridModuleDetails"
          :modules="hybridModules" 
          :highlighted-module="highlightedModule"
        />
      </div>
    </div>
...
const votingDiagram = ref(null)
const hybridDiagram = ref(null)
const votingModuleDetails = ref(null)
const hybridModuleDetails = ref(null)
...
function handleNodeClick(nodeText, nodeMap, moduleDetailsRef) {
...
function addClickHandlersToDiagram(element, nodeMap, moduleDetailsRef) {
  if (!element) {
  ...
  nodeGroups.forEach((group, index) => {
    ...
    if (!nodeText && title) {
      nodeText = title.textContent || title.innerText || ''
    }
    ...
    const shape = group.querySelector('rect, ellipse, polygon, path, circle')
    if (!shape) {
...
// Render diagrams
async function renderDiagram(element, diagramCode, nodeMap, moduleDetailsRef) {
  if (!element) {
