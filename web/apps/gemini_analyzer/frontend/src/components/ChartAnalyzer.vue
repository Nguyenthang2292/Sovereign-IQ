  ...
  <div class="max-w-7xl mx-auto p-4 md:p-6">
    ...
    <div class="glass-panel rounded-xl p-4 mb-6">
      <div class="flex gap-4">
        <button data-testid="mode-single-button" @click="mode = 'single'" :class="[
        ...
        <button data-testid="mode-multi-button" @click="mode = 'multi'" :class="[
          'flex-1 px-4 md:px-6 py-3 rounded-lg font-semibold transition-all duration-300',
          mode === 'multi'
            ? 'btn-gradient text-white hover:shadow-glow-purple hover:scale-[1.02] active:scale-[0.98]'
            : 'bg-gray-700/50 text-gray-300 hover:bg-gray-600/50 border border-gray-600/50'
    ...
    <div class="glass-panel rounded-xl p-4 md:p-6 mb-6">
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          <div class="relative">
            <input data-testid="symbol-input" v-model="form.symbol" type="text"
        ...
        <div v-if="mode === 'single'">
          ...
          <div class="relative">
            ...
            <CustomDropdown data-testid="timeframe-select" v-model="form.timeframe"
              :options="['15m', '30m', '1h', '4h', '1d', '1w']" :placeholder="$t('common.selectTimeframe')"
              :has-left-icon="true" />
          </div>
        </div>
        ...
        <div v-if="mode === 'multi'">
          <label class="block text-sm font-medium text-gray-300 mb-2 flex items-center gap-2">
            <span>🕐</span>
            <span>{{ $t('common.timeframes') }} <span class="text-red-400">{{ $t('common.required') }}</span> {{
              $t('common.commaSeparated') }}</span>
          </label>
          <div class="relative">
            <input data-testid="timeframes-input" v-model="form.timeframes" type="text"
        ...
        <div class="border-t border-gray-700 pt-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.indicators.maPeriods" type="text"
            ...
            <div>
              ...
              <input v-model.number="form.indicators.rsiPeriod" type="number" min="1"
            ...
            <div class="flex items-center gap-3">
              <input v-model="form.indicators.enableMacd" type="checkbox" id="enable-macd"
            ...
            <!-- Bollinger Bands -->
            <div class="flex items-center gap-3">
              <input v-model="form.indicators.enableBb" type="checkbox" id="enable-bb"
                class="w-5 h-5 text-purple-600 bg-gray-700/50 border-gray-600/50 rounded focus:ring-purple-500" />
        ...
        <div class="border-t border-gray-700 pt-6">
          <details class="cursor-pointer">
            <summary class="text-lg font-semibold text-white mb-4">{{ $t('chartAnalyzer.fields.advancedOptions') }}
            </summary>
            ...
            <div class="mt-4 space-y-4">
              <div>
                ...
                <CustomDropdown v-model="form.promptType" :options="promptTypeOptions" option-label="label"
                  option-value="value" :placeholder="$t('chartAnalyzer.fields.promptType')" />
              </div>
              ...
              <div v-if="form.promptType === 'custom'">
                ...
                <textarea v-model="form.customPrompt" rows="4"
                  class="w-full px-4 py-2 bg-gray-700/50 border border-gray-600/50 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 backdrop-blur-sm"
                  :placeholder="$t('chartAnalyzer.fields.customPromptPlaceholder')"></textarea>
              </div>
        ...
        <!-- Submit Button -->
        <div class="pt-4">
          <button @click="handleAnalyze" :disabled="loading || !isFormValid" :class="[
    ...
    <div v-if="result" class="glass-panel rounded-xl p-4 md:p-6">
      ...
      <div v-if="mode === 'single' && result.analysis">
        ...
        <div v-if="result.chart_url" class="mb-6">
          <img :src="result.chart_url" :alt="`Chart ${result.symbol} ${result.timeframe}`"
      ...
      <div v-if="mode === 'multi' && result.aggregated">
        <!-- Aggregated Signal -->
        <div
          class="mb-6 p-4 md:p-6 bg-gradient-to-br from-purple-900/30 to-blue-900/30 rounded-lg border border-purple-500/50 backdrop-blur-sm">
        ...
        <div class="space-y-4">
          ...
          <div v-for="(tfResult, tf) in result.timeframes_results" :key="tf"
            ...
            <div v-if="tfResult.chart_url" class="mt-3">
              <img :src="tfResult.chart_url" :alt="`Chart ${result.symbol} ${tf}`"
                class="w-full rounded-lg border border-gray-700" />
            </div>
          </div>
        </div>
...
interface AnalysisResult {
  ...
  analysis?: string
  chart_url?: string
  // Multi-timeframe specifics
  aggregated?: {
    signal: string
    confidence: number
  }
  timeframes_results?: Record<string, {
    signal: string
    confidence: number
    chart_url?: string
    analysis?: string
...
onUnmounted(() => {
  const finalPoller = logPoller.value
  if (finalPoller) {
    finalPoller.stopPolling()
    logPoller.value = null
  }
})
</script>
