            ...
            try:
                with cli_logger.capture_output():
                    ...
                    df, exchange_id = data_fetcher.fetch_ohlcv_with_fallback_exchange(
                        symbol=request.symbol, timeframe=timeframe, limit=request.limit, check_freshness=False
                    )
                    ...
                    chart_generator = ChartGenerator(
                        figsize=request.chart_figsize, style="dark_background", dpi=request.chart_dpi
                    )
                ...
                with cli_logger.capture_output():
                    ...
                    def fetch_data_func(sym, tf):
                        df, _ = data_fetcher.fetch_ohlcv_with_fallback_exchange(
                            symbol=sym, timeframe=tf, limit=request.limit, check_freshness=False
                        )
                        return df
