    ...
    try:
        ...
        for k in keys:
            if isinstance(value, dict) and k in value:
                ...
                logger.debug(
                    f"Translation key '{key}' not found in default locale '{DEFAULT_LOCALE}', "
                    f"returning key as-is"
                )
                return key
        ...
        logger.warning(
            f"Translation error for key '{key}' in locale '{locale}': {type(e).__name__}: {e}. "
            f"Returning key as-is"
        )
        return key
