...
class BatchScanRequest(BaseModel):
    ...
    max_symbols: Optional[int] = Field(
        default=None, 
        description="Maximum number of symbols to scan (None = all)",
        ge=1
    )
    limit: Optional[int] = Field(
        default=500, 
        description="Number of candles per symbol",
        ge=1, le=5000
    )
    cooldown: Optional[float] = Field(
        default=2.5, 
        description="Cooldown between batch requests in seconds",
        ge=0.0, le=60.0
    )
    charts_per_batch: Optional[int] = Field(
        default=100, 
        description="Number of charts per batch (single TF mode)",
        ge=1, le=1000
    )
