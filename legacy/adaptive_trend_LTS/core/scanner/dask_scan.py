...
def _scan_dask(
    ...
    def fetch_and_process_partition(partition_symbols: list) -> list:
        ...
        # Fix pickling error: Instantiate DataFetcher locally on worker
        # DataFetcher contains thread locks which cannot be pickled for Dask workers
        try:
            from modules.common.core.data_fetcher import DataFetcher
            from modules.common.core.exchange_manager import ExchangeManager

            # Initialize minimal manager for this partition
            exchange_manager = ExchangeManager()
            local_fetcher = DataFetcher(exchange_manager)

            try:
                # Use local fetcher instead of passed (pickled) data_fetcher
                partition_data = _fetch_partition_lazy(partition_symbols, local_fetcher, atc_config)
            ...
            # Fallback if imports fail (should not happen in correct env)
            log_warn("Dask worker failed to import DataFetcher, using fallback logic")
            return []
        except Exception as e:
            log_warn(f"Dask partition error: {e}")
            return []
