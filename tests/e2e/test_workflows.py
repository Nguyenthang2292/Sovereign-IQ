...
def test_hybrid_analysis_workflow():
    ...
    try:
        ...
        ro_signal, ro_confidence = analyzer.get_range_oscillator_signal(symbol="BTC/USDT", timeframe="1h", limit=200)
        print(f"   RO Signal: {ro_signal}, Confidence: {ro_confidence}")

        print("2. Getting SPC signal...")
        try:
            spc_signal, spc_confidence = analyzer.get_spc_signal(symbol="BTC/USDT", timeframe="1h", limit=200)
        ...
        indicator_votes = analyzer.calculate_indicator_votes(
            symbol="BTC/USDT", timeframe="1h", limit=200, use_hmm=True, use_xgboost=True, use_random_forest=True
        )
...
def test_trading_signal_workflow():
    ...
    try:
        ...
        ro_signal = get_range_oscillator_signal(
            data_fetcher=data_fetcher, symbol="BTC/USDT", timeframe="1h", limit=100, df=market_data
        )
...
def test_position_sizing_workflow():
    ...
    try:
        ...
        position_size = calculator.calculate_position_size(account_params=account_params, trade_params=trade_params)
        ...
        assert abs(position_value * account_params["stop_loss_pct"] - risk_amount) < 100, (
            "Risk should be close to target"
        )
