...
from modules.lstm.utils.batch_size import get_optimal_batch_size, _estimate_memory_per_sample
...
class TestGetOptimalBatchSizeCPU:
    ...
    def test_cpu_basic_lstm(self):
        ...
        batch_size = get_optimal_batch_size(device, input_size=50, sequence_length=60, 
                                           model_type='lstm', is_training=True)
    ...
    def test_cpu_lstm_attention(self):
        ...
        device = torch.device('cpu')
        batch_size = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                           model_type='lstm_attention', is_training=True)
    ...
    def test_cpu_cnn_lstm(self):
        ...
        device = torch.device('cpu')
        batch_size = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                           model_type='cnn_lstm', is_training=True)
    ...
    def test_cpu_inference_mode(self):
        ...
        device = torch.device('cpu')
        training_batch = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                               model_type='lstm', is_training=True)
        inference_batch = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                                model_type='lstm', is_training=False)
    ...
    def test_cpu_large_input(self):
        ...
        device = torch.device('cpu')
        batch_size = get_optimal_batch_size(device, input_size=500, sequence_length=200,
                                           model_type='lstm', is_training=True)
    ...
    def test_cpu_invalid_dimensions(self):
        ...
        batch_size = get_optimal_batch_size(device, input_size=0, sequence_length=60,
                                           model_type='lstm', is_training=True)
        assert batch_size >= MIN_BATCH_SIZE
...
class TestGetOptimalBatchSizeEdgeCases:
    ...
    def test_fallback_batch_sizes(self):
        ...
        with patch('torch.cuda.mem_get_info', side_effect=Exception("GPU error")):
            lstm_batch = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                               model_type='lstm', is_training=True)
            attention_batch = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                                     model_type='lstm_attention', is_training=True)
            cnn_batch = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                              model_type='cnn_lstm', is_training=True)
            
            # Check fallback values
            assert lstm_batch == GPU_BATCH_SIZE
            assert attention_batch == max(FALLBACK_MIN_BATCH_LSTM_ATTENTION, 
                                         GPU_BATCH_SIZE // FALLBACK_BATCH_DIVISOR_LSTM_ATTENTION)
            assert cnn_batch == max(FALLBACK_MIN_BATCH_CNN_LSTM,
                                   GPU_BATCH_SIZE // FALLBACK_BATCH_DIVISOR_CNN_LSTM)
    ...
    def test_unknown_model_type_fallback(self):
        ...
        with patch('torch.cuda.mem_get_info', side_effect=Exception("GPU error")):
            batch_size = get_optimal_batch_size(device, input_size=50, sequence_length=60,
                                               model_type='unknown', is_training=True)
            # Should fallback to GPU_BATCH_SIZE
            assert batch_size == GPU_BATCH_SIZE
    ...
    def test_very_large_dimensions(self):
        ...
        device = torch.device('cpu')
        batch_size = get_optimal_batch_size(device, input_size=10000, sequence_length=1000,
                                           model_type='lstm', is_training=True)
        # Should still respect minimum batch size
        assert batch_size >= MIN_BATCH_SIZE
