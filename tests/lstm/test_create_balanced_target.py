...
from config.lstm import TARGET_THRESHOLD_LSTM, NEUTRAL_ZONE_LSTM, FUTURE_RETURN_SHIFT
...
class TestCreateBalancedTarget:
    ...
    def test_create_targets_target_distribution(self):
        ...
        assert all(t in [-1, 0, 1] for t in unique_targets), (
            f"All targets should be -1, 0, or 1, got {unique_targets}"
        )
    ...
    def test_create_targets_large_dataset(self):
        ...
        df = pd.DataFrame({
            'close': [100 + i * 0.001 * np.sin(i/10) for i in range(200)]
        })
