...
class TestPrecomputeAllIndicatorsVectorized:
    ...
    def test_precompute_returns_dict_of_dataframes(self, mock_data_fetcher, sample_dataframe_with_features):
        ...
        calculator = HybridSignalCalculator(
            mock_data_fetcher,
            enabled_indicators=['range_oscillator', 'xgboost']
        )
        
        with patch.object(calculator, '_calc_range_oscillator_vectorized') as mock_osc, \
    ...
    def test_precompute_handles_all_indicators(self, mock_data_fetcher, sample_dataframe_with_features):
        ...
        calculator = HybridSignalCalculator(
            mock_data_fetcher,
            enabled_indicators=['range_oscillator', 'spc', 'xgboost', 'hmm', 'random_forest']
        )
        ...
        with patch('modules.position_sizing.core.hybrid_signal_calculator.IndicatorEngine') as mock_engine_class:
            ...
            with patch('modules.position_sizing.core.hybrid_signal_calculator.apply_directional_labels') as mock_apply_labels:
                ...
                with patch('modules.position_sizing.core.hybrid_signal_calculator.train_and_predict') as mock_train, \
                    ...
                    assert call_count > 1, f"Expected compute_features to be called multiple times, but was called {call_count} times"
                    
                    # train_and_predict should be called multiple times (proving processing continued after first error)
                    assert mock_train.call_count > 1, f"Expected train_and_predict to be called multiple times, but was called {mock_train.call_count} times"
                    ...
                    assert mock_train.call_count >= 2, "Expected at least 2 calls to train_and_predict (first fails, subsequent succeed)"
