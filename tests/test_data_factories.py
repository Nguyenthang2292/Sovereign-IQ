    ...
    from tests.test_data_factories import TestDataFactory
...
class TestDataFactory:
    ...
    def create_oscillator_data(
        ...
        oscillator = pd.Series(
            np.sin(np.linspace(0, frequency * 2 * np.pi, size)) * amplitude,
            index=dates
        )
        
        # Create moving average
        ma = pd.Series(
            np.cumsum(np.ones(size)) / np.arange(1, size + 1),
            index=dates
        ) * 50000  # Scale to realistic price
        
        # Create ATR-like range
        range_atr = pd.Series(
            np.ones(size) * amplitude * 2,
            index=dates
        )
    ...
    def create_test_signals(
        ...
        signals = pd.Series(
            np.random.choice(signal_types, size=size),
            index=dates,
            dtype='int8'
        )
    ...
    def create_edge_case_data(self) -> Dict[str, Any]:
        ...
        # Zero volatility
        edge_cases['zero_vol'] = self.create_ohlc_data(
            size=50, volatility=0.0, cache_key='zero_vol'
        )
        
        # High volatility
        edge_cases['high_vol'] = self.create_ohlc_data(
            size=50, volatility=0.10, cache_key='high_vol'
        )
        
        # Constant price
        edge_cases['constant'] = self.create_ohlc_data(
            size=50, volatility=0.0, trend=0.0, cache_key='constant'
        )
...
FAST_INDICATORS_100 = get_global_factory().create_indicators(
    size=100, cache_key='fast_indicators_100'
)