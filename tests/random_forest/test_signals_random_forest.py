...
class TestTrainRandomForestModel:
    ...
    def test_invalid_inputs(
        self, mock_sufficient_memory, invalid_data, description, request, single_class_dataframe, insufficient_dataframe
    ):
...
class TestApplyConfidenceThreshold:
    ...
    def test_confidence_thresholds(self, test_evaluation_data, threshold, expected):
        ...
        result = apply_confidence_threshold(test_evaluation_data["y_proba"], threshold, test_evaluation_data["classes"])
...
class TestCalculateAndDisplayMetrics:
    ...
    def test_success(self, mock_log_model, test_evaluation_data):
        ...
        calculate_and_display_metrics(test_evaluation_data["y_true"], test_evaluation_data["y_pred"], 0.7)
...
class TestEvaluateModelWithConfidence:
    ...
    def test_success(self, mock_log_model, test_evaluation_data):
        ...
        X_test = pd.DataFrame(np.random.randn(10, len(MODEL_FEATURES)), columns=pd.Index(MODEL_FEATURES))
