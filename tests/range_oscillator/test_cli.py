...
class TestDisplayConfiguration:
    ...
    def test_display_configuration_basic(self, capsys):
        ...
        display_configuration(timeframe="1h", limit=500, min_signal=0.01, strategies=None)
    ...
    def test_display_configuration_with_strategies(self, capsys):
        ...
        display_configuration(timeframe="15m", limit=1000, min_signal=0.05, strategies=[2, 3, 4], max_symbols=50)
    ...
    def test_display_configuration_with_max_symbols(self, capsys):
        ...
        display_configuration(timeframe="1h", limit=500, min_signal=0.01, strategies=None, max_symbols=100)
...
class TestDisplayFinalResults:
    ...
    def test_display_final_results_without_confidence(self, capsys):
        ...
        long_signals = pd.DataFrame(
            {"symbol": ["BTC/USDT"], "signal": [0.5], "price": [50000.0], "exchange": ["binance"]}, index=dates[:1]
        )

        short_signals = pd.DataFrame(
            {"symbol": ["ETH/USDT"], "signal": [-0.3], "price": [3000.0], "exchange": ["binance"]}, index=dates[1:2]
        )
    ...
    def test_display_final_results_input_validation(self):
        ...
        with pytest.raises(ValueError):
            display_final_results(
                long_signals=incomplete_df, short_signals=empty_df, original_long_count=0, original_short_count=0
            )
    ...
    def test_display_final_results_with_missing_keys(self, capsys):
        ...
        short_signals = pd.DataFrame(
            {"symbol": ["ADA/USDT"], "signal": [-0.4], "price": [0.5], "exchange": ["binance"]}, index=dates[2:3]
        )
