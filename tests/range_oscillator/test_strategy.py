...
class TestStrategy2:
    ...
    def test_strategy2_validation(self, sample_ohlc_data):
        ...
        with pytest.raises(ValueError, match="min_bars_above_zero must be > 0"):
            generate_signals_sustained_strategy(high=high, low=low, close=close, min_bars_above_zero=-1)
    ...
    def test_strategy2_conflict_long_short(self):
        ...
        signals, strength = generate_signals_sustained_strategy(
            oscillator=oscillator_with_nan,
            ma=ma,
            range_atr=range_atr,
            min_bars_above_zero=3,
            min_bars_below_zero=3,
...
class TestStrategy3:
    ...
    def test_strategy3_crossover_with_confirmation(self):
        ...
        signals, strength = generate_signals_crossover_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, confirmation_bars=2, oscillator_threshold=0.0
        )
    ...
    def test_strategy3_no_confirmation(self):
        ...
        signals, strength = generate_signals_crossover_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, confirmation_bars=3, oscillator_threshold=0.0
        )
...
class TestStrategy4:
    ...
    def test_strategy4_strong_momentum(self):
        ...
        signals, strength = generate_signals_momentum_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, momentum_period=3, momentum_threshold=5.0
        )
    ...
    def test_strategy4_insufficient_data(self):
        ...
        with pytest.raises(ValueError, match="momentum_period.*must be < data length"):
            generate_signals_momentum_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr, momentum_period=10)
...
class TestStrategy5:
    ...
    def test_strategy5_majority_vote(self):
        ...
        signals, strength = generate_signals_combined_all_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, use_sustained=True, use_crossover=True, use_momentum=True
        )
...
class TestStrategy5Integration:
    ...
    def test_fallback_to_strategy1_when_no_methods_enabled(self, sample_ohlc_data):
        ...
        signals1, strength1 = generate_signals_basic_strategy(high=high, low=low, close=close)
...
class TestStrategy6:
    ...
    def test_strategy6_basic(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_breakout_strategy(
            high=high, low=low, close=close, upper_threshold=100.0, lower_threshold=-100.0, require_confirmation=True
        )
    ...
    def test_strategy6_with_precalculated(self, sample_ohlc_data, sample_oscillator_data):
        ...
        signals, strength = generate_signals_breakout_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, upper_threshold=80.0, detect_exhaustion=False
        )
    ...
    def test_strategy6_validation(self, sample_ohlc_data):
        ...
        with pytest.raises(ValueError, match="confirmation_bars must be > 0"):
            generate_signals_breakout_strategy(
                high=high, low=low, close=close, require_confirmation=True, confirmation_bars=0
            )

        with pytest.raises(ValueError, match="exhaustion_threshold must be >= 0"):
            generate_signals_breakout_strategy(
                high=high, low=low, close=close, detect_exhaustion=True, exhaustion_threshold=-1.0
            )
        ...
        with pytest.raises(ValueError, match="upper_threshold.*must be > lower_threshold"):
            generate_signals_breakout_strategy(
                high=high, low=low, close=close, upper_threshold=50.0, lower_threshold=100.0
            )
    ...
    def test_strategy6_dynamic_exhaustion_edge_cases(self):
        ...
        signals, strength = generate_signals_breakout_strategy(
            oscillator=short_oscillator,
            ma=short_ma,
            range_atr=short_range_atr,
            detect_exhaustion=True,
            use_dynamic_exhaustion=True,
            base_exhaustion_threshold=150.0,
            exhaustion_atr_multiplier=1.0,
...
class TestStrategy7:
    ...
    def test_strategy7_basic(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_divergence_strategy(
            high=high, low=low, close=close, lookback_period=30, min_swing_bars=5
        )
    ...
    def test_strategy7_requires_price_data(self, sample_oscillator_data):
        ...
        with pytest.raises(ValueError, match="high, low, close are required"):
            generate_signals_divergence_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy7_validation(self, sample_ohlc_data):
        ...
        with pytest.raises(ValueError, match="min_swing_bars must be > 0"):
            generate_signals_divergence_strategy(high=high, low=low, close=close, min_swing_bars=0)

        with pytest.raises(ValueError, match="lookback_period must be > 0"):
            generate_signals_divergence_strategy(high=high, low=low, close=close, lookback_period=0)

        with pytest.raises(ValueError, match="min_divergence_strength must be >= 0"):
            generate_signals_divergence_strategy(high=high, low=low, close=close, min_divergence_strength=-1.0)
...
class TestStrategy8:
    ...
    def test_strategy8_basic(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_trend_following_strategy(
            high=high, low=low, close=close, trend_filter_period=10, oscillator_threshold=20.0
        )
    ...
    def test_strategy8_with_precalculated(self, sample_ohlc_data, sample_oscillator_data):
        ...
        signals, strength = generate_signals_trend_following_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, require_consistency=False
        )
    ...
    def test_strategy8_validation(self, sample_ohlc_data):
        ...
        with pytest.raises(ValueError, match="trend_filter_period must be > 0"):
            generate_signals_trend_following_strategy(high=high, low=low, close=close, trend_filter_period=0)

        with pytest.raises(ValueError, match="oscillator_threshold must be >= 0"):
            generate_signals_trend_following_strategy(high=high, low=low, close=close, oscillator_threshold=-1.0)
...
class TestStrategy9:
    ...
    def test_strategy9_basic(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_mean_reversion_strategy(
            high=high, low=low, close=close, extreme_threshold=80.0, zero_cross_threshold=10.0
        )
    ...
    def test_strategy9_with_precalculated(self, sample_ohlc_data, sample_oscillator_data):
        ...
        signals, strength = generate_signals_mean_reversion_strategy(
            oscillator=oscillator, ma=ma, range_atr=range_atr, min_extreme_bars=3
        )
    ...
    def test_strategy9_validation(self, sample_ohlc_data):
        ...
        with pytest.raises(ValueError, match="extreme_threshold must be >= 0"):
            generate_signals_mean_reversion_strategy(high=high, low=low, close=close, extreme_threshold=-1.0)

        with pytest.raises(ValueError, match="zero_cross_threshold must be >= 0"):
            generate_signals_mean_reversion_strategy(high=high, low=low, close=close, zero_cross_threshold=-1.0)

        with pytest.raises(ValueError, match="min_extreme_bars must be > 0"):
            generate_signals_mean_reversion_strategy(high=high, low=low, close=close, min_extreme_bars=0)

        with pytest.raises(ValueError, match="transition_bars must be > 0"):
            generate_signals_mean_reversion_strategy(high=high, low=low, close=close, transition_bars=0)
...
class TestSignalSummary:
    ...
    def test_get_signal_summary_basic(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_basic_strategy(high=high, low=low, close=close)
...
class TestEdgeCases:
    ...
    def test_all_strategies_with_short_data(self):
        ...
        signals, strength = generate_signals_basic_strategy(high=high, low=low, close=close, length=5)
    ...
    def test_strategies_with_nan_values(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_basic_strategy(high=high, low=low, close=close_with_nan)
...
class TestConflictHandling:
    ...
    def test_strategy2_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_sustained_strategy(high=high, low=low, close=close)
    ...
    def test_strategy3_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_crossover_strategy(high=high, low=low, close=close)
    ...
    def test_strategy4_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_momentum_strategy(high=high, low=low, close=close)
    ...
    def test_strategy5_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_combined_all_strategy(high=high, low=low, close=close)
    ...
    def test_strategy6_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_breakout_strategy(high=high, low=low, close=close)
    ...
    def test_strategy7_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_divergence_strategy(high=high, low=low, close=close)
    ...
    def test_strategy8_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_trend_following_strategy(high=high, low=low, close=close)
    ...
    def test_strategy9_conflict_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_mean_reversion_strategy(high=high, low=low, close=close)
...
class TestNaNHandling:
    ...
    def test_strategy2_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_sustained_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy3_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_crossover_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy4_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_momentum_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy5_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_combined_all_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy6_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_breakout_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy7_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_divergence_strategy(high=high, low=low, close=close_with_nan)
    ...
    def test_strategy8_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_trend_following_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
    ...
    def test_strategy9_nan_handling(self, sample_ohlc_data):
        ...
        signals, strength = generate_signals_mean_reversion_strategy(oscillator=oscillator, ma=ma, range_atr=range_atr)
