...
def test_main_function_basic_flow(
    ...
    mock_data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(return_value=(sample_df_with_indicators, "binance"))
...
@patch("modules.xgboost.cli.main.ExchangeManager")
@patch("modules.xgboost.cli.main.DataFetcher")
def test_main_with_no_data(mock_data_fetcher_class, mock_exchange_manager_class, mock_parse_args):
...
def test_main_resolve_input_calls(mock_resolve_input, mock_parse_args, sample_df_with_indicators):
...
def test_run_once_logic(sample_df_with_indicators, mock_model):
    ...
    with (
        patch("modules.xgboost.cli.main.DataFetcher") as MockDataFetcher,
        patch("modules.xgboost.cli.main.IndicatorEngine") as MockIndicatorEngine,
...
def test_prediction_output_formatting(sample_df_with_indicators):
    ...
    from config import ID_TO_LABEL, LABEL_TO_ID, TARGET_LABELS
