...
def test_scan_all_symbols_sequential_mode(mock_trend_sign, mock_compute_atc):
    ...
    mock_fetcher.fetch_ohlcv_with_fallback_exchange = MagicMock(return_value=(mock_df, "binance"))
    ...
    long_signals, short_signals = scan_all_symbols(mock_fetcher, config, execution_mode="sequential", max_symbols=2)
...
def test_scan_all_symbols_empty_dataframe(mock_trend_sign, mock_compute_atc):
    ...
    symbols = ["BTC/USDT"]
    mock_fetcher.list_binance_futures_symbols = MagicMock(return_value=symbols)
    mock_fetcher.fetch_ohlcv_with_fallback_exchange = MagicMock(return_value=(pd.DataFrame(), "binance"))

    long_signals, short_signals = scan_all_symbols(mock_fetcher, config, execution_mode="sequential", max_symbols=1)
...
def test_scan_all_symbols_max_symbols_limit(mock_trend_sign, mock_compute_atc):
    ...
    long_signals, short_signals = scan_all_symbols(mock_fetcher, config, execution_mode="sequential", max_symbols=3)
