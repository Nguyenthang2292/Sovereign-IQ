...
def test_import_overhead():
    ...
    import_time = time_operation(
        "Import at module level", lambda: __import__("modules.range_oscillator.strategies.basic")
    )
...
def test_strategy_execution():
    ...
    def execute_strategy():
        return generate_signals_basic_strategy(high=high, low=low, close=close, length=20, mult=2.0)
...
def test_assertion_overhead():
    ...
    comprehensive_time = time_operation(
        "Comprehensive assertions", lambda: comprehensive_assertions((high, low, close)), iterations=10
    )
...
def test_pytest_overhead():
    ...
    stdout_str = collect_result.stdout.decode("utf-8", errors="ignore") if collect_result.stdout else ""
    print(f"Tests collected: {stdout_str.count('test_')}")
    print()
