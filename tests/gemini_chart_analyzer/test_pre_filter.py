...
from modules.gemini_chart_analyzer.cli.pre_filter import (
    pre_filter_symbols_with_voting,
    pre_filter_symbols_with_hybrid
)
...
def mock_voting_analyzer():
    ...
    analyzer.long_signals_final = pd.DataFrame({
        'symbol': ['BTC/USDT', 'ETH/USDT'],
        'weighted_score': [0.9, 0.8]
    })
    analyzer.short_signals_final = pd.DataFrame({
        'symbol': ['BNB/USDT'],
        'weighted_score': [0.7]
    })
...
def mock_hybrid_analyzer():
    ...
    analyzer.long_signals_confirmed = pd.DataFrame({
        'symbol': ['BTC/USDT', 'ETH/USDT']
    })
    analyzer.short_signals_confirmed = pd.DataFrame({
        'symbol': ['BNB/USDT']
    })
...
class TestPreFilterWithVoting:
    ...
    def test_pre_filter_voting_no_atc_signals(self, sample_symbols):
        ...
        with patch('modules.gemini_chart_analyzer.cli.pre_filter.ExchangeManager') as mock_exchange_mgr, \
