...
class TestSignalAggregatorInit:
    ...
    def test_init_custom_weights(self, custom_aggregator):
        ...
        assert "15m" not in DEFAULT_TIMEFRAME_WEIGHTS or custom_aggregator.timeframe_weights[
            "15m"
        ] != DEFAULT_TIMEFRAME_WEIGHTS.get("15m", 0)
...
class TestAggregateSignals:
    ...
    def test_unknown_timeframe_weight(self, default_aggregator):
        ...
        signals = {"unknown_tf": {"signal": "LONG", "confidence": 0.7}}
        result = default_aggregator.aggregate_signals(signals)
    ...
    def test_breakdown_structure(self, default_aggregator):
        ...
        signals = {"15m": {"signal": "LONG", "confidence": 0.7}, "1h": {"signal": "SHORT", "confidence": 0.6}}
        result = default_aggregator.aggregate_signals(signals)
...
class TestCalculateWeightedConfidence:
    ...
    def test_multiple_signals_same_weight(self, default_aggregator):
        ...
        signals = [("15m", 0.6, 0.5), ("1h", 0.8, 0.5)]
        result = default_aggregator._calculate_weighted_confidence(signals)
    ...
    def test_zero_total_weight(self, default_aggregator):
        ...
        signals = [("15m", 0.7, 0.0), ("1h", 0.8, 0.0)]
        result = default_aggregator._calculate_weighted_confidence(signals)
        assert result == 0.0
    ...
    def test_all_nan_values(self, default_aggregator):
        ...
        # moved import to module level
        signals = [("15m", float("nan"), float("nan")), ("1h", float("nan"), float("nan"))]
        result = default_aggregator._calculate_weighted_confidence(signals)
    ...
    def test_result_always_in_valid_range(self, default_aggregator):
        ...
        signals = [("15m", 1.0, 1.0), ("1h", 0.0, 1.0)]
        result = default_aggregator._calculate_weighted_confidence(signals)
        assert 0.0 <= result <= 1.0
        assert not math.isnan(result)
        assert not math.isinf(result)
...
class TestDetermineFinalSignal:
    ...
    def test_long_clear_winner(self, default_aggregator):
        ...
        signal, confidence = default_aggregator._determine_final_signal(
            long_weighted_conf=0.8, short_weighted_conf=0.3, none_weighted_conf=0.2
        )
        assert signal == "LONG"
    ...
    def test_short_clear_winner(self, default_aggregator):
        ...
        signal, confidence = default_aggregator._determine_final_signal(
            long_weighted_conf=0.3, short_weighted_conf=0.8, none_weighted_conf=0.2
        )
        assert signal == "SHORT"
        assert confidence == 0.8
    ...
    def test_equal_long_short(self, default_aggregator):
        ...
        signal, confidence = default_aggregator._determine_final_signal(
            long_weighted_conf=0.6, short_weighted_conf=0.6, none_weighted_conf=0.3
        )
        assert signal == "NONE"
    ...
    def test_all_low_confidences(self, default_aggregator):
        ...
        signal, confidence = default_aggregator._determine_final_signal(
            long_weighted_conf=0.2, short_weighted_conf=0.3, none_weighted_conf=0.4
        )
        assert signal == "NONE"
