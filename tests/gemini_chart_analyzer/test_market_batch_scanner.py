...
def mock_scanner_dependencies():
    ...
    with (
        patch("modules.gemini_chart_analyzer.core.scanners.market_batch_scanner.ExchangeManager") as mock_exchange,
        patch(
            "modules.gemini_chart_analyzer.core.scanners.market_batch_scanner.PublicExchangeManager"
        ) as mock_public_exchange,
...
class TestMarketBatchScannerInit:
    ...
    def test_init_custom_params(self, mock_scanner_dependencies):
        ...
        scanner = MarketBatchScanner(charts_per_batch=50, cooldown_seconds=5.0, quote_currency="BTC", min_candles=30)
...
class TestMarketBatchScannerGetSymbols:
    ...
    def test_get_all_symbols_success(self, mock_scanner_dependencies, sample_symbols):
        ...
        for symbol in sample_symbols:
            mock_markets[symbol] = {"quote": "USDT", "active": True, "type": "spot"}
    ...
    def test_get_all_symbols_filters_inactive(self, mock_scanner_dependencies):
        ...
        mock_exchange = mock_scanner_dependencies["exchange"]
        mock_exchange.load_markets.return_value = mock_markets
        scanner.public_exchange_manager.connect_to_exchange_with_no_credentials = Mock(return_value=mock_exchange)
...
class TestMarketBatchScannerFetchData:
    ...
    def test_fetch_batch_data_success(self, mock_scanner_dependencies):
        ...
        scanner.data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(return_value=(sample_df, "binance"))
    ...
    def test_fetch_batch_data_insufficient_data(self, mock_scanner_dependencies):
        ...
        small_df = pd.DataFrame(
            {"open": [1] * 10, "high": [2] * 10, "low": [0.5] * 10, "close": [1.5] * 10, "volume": [1000] * 10},
            index=dates,
        )

        symbols = ["BTC/USDT"]
        scanner.data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(return_value=(small_df, "binance"))
    ...
    def test_fetch_batch_data_custom_min_candles(self, mock_scanner_dependencies):
        ...
        symbols = ["BTC/USDT"]
        scanner.data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(return_value=(medium_df, "binance"))
        ...
        scanner.data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(return_value=(enough_df, "binance"))
    ...
    def test_fetch_batch_data_error_handling(self, mock_scanner_dependencies):
        ...
        symbols = ["BTC/USDT", "ETH/USDT"]
        scanner.data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(side_effect=Exception("Fetch error"))
...
class TestMarketBatchScannerSaveResults:
    ...
    def test_save_results(self, mock_scanner_dependencies, tmp_path):
        ...
        summary = {"total_symbols": 3, "scanned_symbols": 3, "long_count": 1, "short_count": 1, "none_count": 1}
...
class TestMarketBatchScannerCleanup:
    ...
    def test_cleanup_handles_errors(self, mock_scanner_dependencies):
        ...
        scanner.exchange_manager.cleanup_unused_exchanges = MagicMock(side_effect=Exception("Cleanup error"))
    ...
    def test_cleanup_with_all_errors(self, mock_scanner_dependencies):
        ...
        scanner.exchange_manager.cleanup_unused_exchanges = MagicMock(side_effect=Exception("Auth cleanup error"))
        scanner.public_exchange_manager.cleanup_unused_exchanges = MagicMock(
...
class TestMarketBatchScannerInitialSymbols:
    ...
    def test_scan_market_with_initial_symbols(self, mock_scanner_dependencies):
        ...
        with patch(
            "modules.gemini_chart_analyzer.core.utils.chart_paths.get_analysis_results_dir", return_value="/tmp/test"
        ):
            scanner._save_results = Mock(return_value="/tmp/test/results.json")

            # Call scan_market with initial_symbols
            results = scanner.scan_market(timeframe="1h", initial_symbols=pre_filtered_symbols, limit=500)
    ...
    def test_scan_market_without_initial_symbols(self, mock_scanner_dependencies, sample_symbols):
        ...
        with patch(
            ...
            results = scanner.scan_market(timeframe="1h", limit=500)
