...
"""
🚀 Mức độ 2: Test Fixture Optimization

File này chứa các optimized fixtures để tăng tốc độ test suite.
"""
...
# ==========================================================
# 🚀 SESSION-SCOPED FIXTURES (Tạo 1 lần, dùng cho tất cả tests)
# ==========================================================
...
def mock_data_fetcher(cached_ohlcv_data):
    ...
    data_fetcher.fetch_ohlcv_with_fallback_exchange = Mock(
        return_value=(cached_ohlcv_data, "binance")
    )
...
def mock_trained_model():
    ...
    model.predict_proba = Mock(
        return_value=np.array([[0.2, 0.8], [0.6, 0.4], [0.7, 0.3], [0.5, 0.5], [0.1, 0.9]])
    )
    return model
...
def mock_xgboost_trainer():
    ...
    trainer = Mock()
    trainer.train = Mock(return_value={
...
def test_helpers():
    ...
    return {
        ...
        'create_ohlc_with_signals': lambda size=100: (
            ...
            pd.DataFrame({'open': [100], 'high': [102], 'low': [98], 'close': [100], 'volume': [1000], 'signal': [1], 'confidence': [0.8]})
        ),
