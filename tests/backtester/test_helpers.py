...
def mock_all_signal_calculators(
    osc_signal=1, osc_confidence=0.7,
    spc_signal=1, spc_confidence=0.6,
    xgb_signal=1, xgb_confidence=0.8,
    hmm_signal=1, hmm_confidence=0.65,
    rf_signal=1, rf_confidence=0.75,
):
    """
    ...
    """
    with patch('core.signal_calculators.get_range_oscillator_signal', return_value=(osc_signal, osc_confidence)), \
...
def mock_no_signals():
    ...
    with mock_all_signal_calculators(
        osc_signal=0, osc_confidence=0.0,
        spc_signal=0, spc_confidence=0.0,
        xgb_signal=0, xgb_confidence=0.0,
        hmm_signal=0, hmm_confidence=0.0,
        rf_signal=0, rf_confidence=0.0,
    ):
        yield
