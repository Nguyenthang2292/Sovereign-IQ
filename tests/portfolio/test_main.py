...
from config import BENCHMARK_SYMBOL, DEFAULT_VAR_CONFIDENCE
...
def mock_data_fetcher():
    ...
    mock_df = pd.DataFrame({"timestamp": pd.date_range("2024-01-01", periods=100, freq="1h"), "close": [100.0] * 100})
...
def test_portfolio_manager_calculate_portfolio_var(portfolio_manager, mock_risk_calculator):
    ...
    var = portfolio_manager.calculate_portfolio_var(confidence=0.95, lookback_days=60)
...
def test_portfolio_manager_find_best_hedge_candidate(portfolio_manager, mock_data_fetcher):
    ...
    with (
        ...
        result = portfolio_manager.find_best_hedge_candidate(total_delta=100.0, total_beta_delta=150.0)
