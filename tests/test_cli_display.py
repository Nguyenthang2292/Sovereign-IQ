...
class TestDisplayConfig:
    ...
    def test_display_config_with_spc(self, mock_log_data, mock_log_progress, mock_display_config):
        ...
        assert re.search(r'K:\s*2\b', combined_call_string), \
            "SPC parameter 'K: 2' not found in log_data calls"
...
class TestDisplayVotingMetadata:
    ...
    def test_display_voting_metadata_long_signals(self, mock_log_data, mock_log_progress):
        ...
        df = pd.DataFrame({
            ...
            'weighted_impact': [
                {'spc': 15.0, 'xgboost': 10.0},
                {'spc': 18.0, 'xgboost': 0.0}
            ]
        })
        ...
        assert "75.00%" in logged_text or "0.75" in logged_text, \
            f"weighted_score 0.75 (75.00%) not found in logged messages"
        assert "65.00%" in logged_text or "0.65" in logged_text, \
            f"weighted_score 0.65 (65.00%) not found in logged messages"
        ...
        assert "spc" in logged_text.lower() or "SPC" in logged_text, \
            f"'spc' not found in voting_breakdown logged messages"
        assert "30.0" in logged_text or "30.00%" in logged_text, \
            f"voting_breakdown contribution '30.0' not found in logged messages"
        ...
        symbol_calls = [
            call for call in mock_log_data.call_args_list
    ...
    def test_display_voting_metadata_with_spc_debug(self, mock_log_data, mock_log_progress):
        ...
        assert 'spc' in all_logs.lower() or 'SPC' in all_logs, \
            "SPC indicator not found in logged messages"
        ...
        assert '[DEBUG]' in all_logs or 'DEBUG' in all_logs, \
            "SPC debug marker '[DEBUG]' not found in logged messages"
        ...
        assert 'CT=' in all_logs, \
            "SPC Cluster Transition signal (CT=) not found in debug logs"
        assert 'RF=' in all_logs, \
            "SPC Regime Following signal (RF=) not found in debug logs"
        assert 'MR=' in all_logs, \
            "SPC Mean Reversion signal (MR=) not found in debug logs"
        
        # Verify strength values are logged (they should appear as strength=0.00)
        assert 'strength=' in all_logs.lower(), \
            "SPC strength values not found in debug logs"
        
        # Verify specific SPC signal values are logged
        assert 'CT=0' in all_logs, \
            "SPC Cluster Transition signal value (0) not found in debug logs"
        assert 'RF=0' in all_logs, \
            "SPC Regime Following signal value (0) not found in debug logs"
        assert 'MR=0' in all_logs, \
            "SPC Mean Reversion signal value (0) not found in debug logs"
