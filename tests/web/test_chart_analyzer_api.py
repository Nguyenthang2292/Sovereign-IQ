...
def poll_until_status(client, session_id, expected_status, timeout=2.0, interval=0.04):
    ...
    while time.time() - start < timeout:
        ...
        if status == "error" and expected_status != "error":
            raise AssertionError(
                f"Status endpoint reported 'error': {data!r}"
            )
...
class TestSingleAnalysisEndpoint:
    ...
    def test_success_with_no_cleanup(self, client, chart_analyzer_mocks):
        ...
        with patch('web.api.chart_analyzer._cleanup_old_charts') as mock_cleanup:
            ...
            assert response.status_code == 200, f"Unexpected response code: {response.status_code}, response: {response.text}"
            data = response.json()
    ...
    def test_success_signal_extraction_long(self, client, chart_analyzer_mocks):
        ...
        assert result_data["status"] == "completed", (
            f"Task did not complete successfully: {result_data}"
        )
    ...
    def test_success_with_custom_figsize_and_dpi(self, client, chart_analyzer_mocks):
        ...
        assert call_kwargs.get('dpi') == 200, (
            f"dpi incorrect: expected 200, got {call_kwargs.get('dpi')}"
        )
...
class TestMultiAnalysisEndpoint:
    ...
    def test_success_with_indicators_config(self, client, multi_timeframe_mocks):
        ...
        mocks['mtf'].analyze_deep.return_value = {
            'timeframes': {
                '1h': {'analysis': '1h analysis', 'signal': 'NONE', 'confidence': 0.0}
            },
    ...
    def test_error_invalid_timeframes(self, client):
        ...
        with patch('modules.gemini_chart_analyzer.core.utils.validate_timeframes', return_value=(False, "Invalid timeframe format")):
            request_data = {
                "symbol": "BTC/USDT",
                "timeframes": ["invalid"]
            }
    ...
    def test_success_single_timeframe_in_multi(self, client, multi_timeframe_mocks):
        ...
        mocks['mtf'].analyze_deep.return_value = {
            'timeframes': {
                '1h': {'analysis': '1h analysis', 'signal': 'LONG', 'confidence': 0.7}
            },
...
class TestChartAnalyzerBackgroundThread:
    ...
    def test_analyze_single_returns_session_id_immediately(self, client, chart_analyzer_mocks):
        ...
        assert isinstance(data.get("session_id"), str) and len(data["session_id"]) > 0, "session_id missing or not string"
        ...
        assert incomplete_response.status_code in (400, 422), f"Expected 400/422 for bad request, got {incomplete_response.status_code}"
    ...
    def test_analyze_multi_returns_session_id_immediately(self, client, multi_timeframe_mocks):
        ...
        mocks['mtf'].analyze_deep.return_value = {
            'timeframes': {
                '1h': {'analysis': 'Test', 'signal': 'LONG', 'confidence': 0.7}
            },
