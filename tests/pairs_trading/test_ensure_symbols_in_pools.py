...
def test_ensure_symbols_in_candidate_pools_basic():
    ...
    performance_df = pd.DataFrame({"symbol": ["BTC/USDT", "ETH/USDT", "SOL/USDT"], "score": [0.5, -0.3, 0.2]})
    ...
    best_updated, worst_updated = ensure_symbols_in_candidate_pools(performance_df, best_df, worst_df, ["SOL/USDT"])
...
def test_ensure_symbols_in_candidate_pools_negative_score():
    ...
    performance_df = pd.DataFrame({"symbol": ["BTC/USDT", "ADA/USDT"], "score": [0.5, -0.4]})
    best_df = pd.DataFrame({"symbol": ["BTC/USDT"], "score": [0.5]})
    worst_df = pd.DataFrame(columns=["symbol", "score"])

    # ADA/USDT is negative score (-0.4), should be added to worst_df
    best_updated, worst_updated = ensure_symbols_in_candidate_pools(performance_df, best_df, worst_df, ["ADA/USDT"])
...
def test_ensure_symbols_in_candidate_pools_already_present():
    ...
    performance_df = pd.DataFrame({"symbol": ["BTC/USDT"], "score": [0.5]})
    best_df = pd.DataFrame({"symbol": ["BTC/USDT"], "score": [0.5]})
    worst_df = pd.DataFrame(columns=["symbol", "score"])

    # BTC/USDT already in best_df
    best_updated, worst_updated = ensure_symbols_in_candidate_pools(performance_df, best_df, worst_df, ["BTC/USDT"])
...
def test_ensure_symbols_in_candidate_pools_missing_symbol():
    ...
    best_updated, worst_updated = ensure_symbols_in_candidate_pools(performance_df, best_df, worst_df, ["UNKNOWN/USDT"])
...
def test_ensure_symbols_in_candidate_pools_empty_target():
    ...
    best_updated, worst_updated = ensure_symbols_in_candidate_pools(performance_df, best_df, worst_df, [])
