...
from modules.lstm.models.model_utils import load_model_and_scaler, get_latest_signal
...
def load_all_models(allow_missing_scaler: bool = False) -> List[Tuple[Path, object, object, int]]:
    ...
    for model_path in model_files:
        try:
            ...
            if model is None:
                log_warn(f"  ⚠️  Failed to load model")
                continue
            
            if scaler is None:
                if allow_missing_scaler:
                    log_warn(f"  ⚠️  Scaler not found in checkpoint for {model_path.name}")
                    log_warn(f"  ⚠️  Continuing without scaler (predictions may be unreliable)")
                else:
                    log_warn(f"  ⚠️  Scaler not found in checkpoint for {model_path.name}")
                    log_warn(f"  ⚠️  Skipping this model (use --allow-missing-scaler to test anyway)")
                    continue
            
            if look_back is None:
                log_warn(f"  ⚠️  Look_back not found in checkpoint, using default")
    ...
    try:
        ...
        if len(prediction_probs) == 3:
            ...
            probs_dict = {
                ...
                'NEUTRAL': float(prediction_probs[1]) if len(prediction_probs) > 1 else float(prediction_probs[0]) if len(prediction_probs) > 0 else 0.0,
...
def print_aggregated_results(all_results: List[Dict], results_by_model: Dict[str, List[Dict]], models: List) -> None:
    ...
    if valid_confidences:
        ...
        print(f"Low confidence tests (< {CONFIDENCE_THRESHOLD}): {low_confidence_count}/{len(valid_confidences)} ({low_confidence_count/len(valid_confidences)*100:.1f}%)")
    ...
    for model_name, model_results in sorted(results_by_model.items()):
        ...
        print(f"{display_name:<60} {test_count:<8} {buy_count:<8} {neutral_count:<10} {sell_count:<8} {avg_conf:<10.3f}")
    
    # Check for NEUTRAL bias
    if signal_counts:
        neutral_ratio = signal_counts.get('NEUTRAL', 0) / (total_tests - error_count) if (total_tests - error_count) > 0 else 0
...
def print_results_table(results: List[Dict]) -> None:
    ...
    print(f"NEUTRAL signals: {signal_counts.get('NEUTRAL', 0)} ({signal_counts.get('NEUTRAL', 0)/len(results)*100:.1f}%)")
...
def main():
    ...
    if allow_missing_scaler:
        print(f"{Fore.YELLOW}⚠️  Mode: Allowing models without scaler (predictions may be unreliable){Style.RESET_ALL}\n")
    ...
    try:
        ...
        available_timeframes = DEFAULT_TIMEFRAMES_FOR_TRAINING_DL if DEFAULT_TIMEFRAMES_FOR_TRAINING_DL else ['15m', '1h', '4h', '1d']
