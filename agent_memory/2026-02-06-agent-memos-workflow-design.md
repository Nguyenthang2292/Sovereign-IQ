...
- **Store:** Tự động qua **git hook** (post-commit): mỗi commit → script tóm tắt (message + diff ngắn) → `add_message` lên MemOS. Thêm **CLI** để ghi thủ công: `python -m agent_memory store "tóm tắt workflow"`.
...
- **Module:** `agent_memory/` (ở root repo, cùng cấp với `modules/`).
...
agent_memory/
  ...
  cli.py          # Entry: recall, store (python -m agent_memory recall|store [...])
README.md
```
...
- Implement `agent_memory/`: client, config, recall, store, cli.
- Thêm Cursor rule: "Khi bắt đầu session, đọc `.cursor/agent_memory_context.md` để lấy quy ước và workflow gần đây."
- Gợi ý task/script "Refresh agent memory" (chạy `python -m agent_memory recall`) khi mở project hoặc khi cần.
